...
class Scanner:
    ...
    def scanLine(self, Line, SecondsPerPoint, return_speed=None):
        time.sleep(0.1)
        return (
            1000 * np.sin(Line[0, :]) * np.sin(Line[1, :]) * np.exp(-Line[2, :] ** 2)
        ).astype(int)
...
class Microwave:
    ...
    def setOutput(self, power, frequency):
        logging.getLogger().debug(
            "Setting microwave to p=" + str(power) + " f=" + str(frequency) + "."
        )

    def initSweep(self, f, p):
        logging.getLogger().debug(
            "Setting microwave to sweep between frequencies %e .. %e with power %f."
            % (f[0], f[-1], p[0])
        )
...
if os.access("hardware/custom_api.py", os.F_OK):
    exec(
        compile(
            open("hardware/custom_api.py", "rb").read(),
            "hardware/custom_api.py",
            "exec",
        )
    )
