...
def Scanner():
    ...
    return Scanner(
        ...
        z_range=(0, 100.0),
        v_range=(0.0, 10.0),
    )  # the voltage range here is only used to give a restriction to the Multiboard class in nidaq.py
...
def Counter():
    ...
    return PulseTrainCounter(
        CounterIn="/Dev1/Ctr3",  # Why here is CTR 3 not CTR 1?
        CounterOut="/Dev1/Ctr0",
        TickSource="/Dev1/PFI3",
    )  # CTR 1 SRC
...
def Microwave():
    ...
    return microwave_sources.SMIQ(
        visa_address="GPIB0::7"
    )  # the one below awg610 on the shelf
...
def MicrowaveB():
    ...
    return microwave_sources.SMIQ(
        visa_address="GPIB0::29"
    )  # the one at the bottom of shelf
...
def MicrowaveC():
    ...
    return microwave_sources.SMIQ(
        visa_address="GPIB0::27"
    )  # the one on the shelf above table
...
@singleton
def RFSource():
    from . import rf_source

    return rf_source.Rigol822(
...
def AWG610():
    ...
    return AWGManager610(
        gpib="GPIB0::1::INSTR", ftp="192.168.1.7", socket=("192.168.1.7", 4000)
    )  # address unsure since socket seems not used
