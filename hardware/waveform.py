...
MAX_POINTS = {'AWG520':4194048}
MIN_POINTS = {'AWG520':256}
...
class Envelop(Pulse):
    ...
    def fourier(self, segment):
        ...
        ft = self.amp_li[:,None]*np.cos(2*np.pi*self.freq_li[:,None]*segment + phase[:,None])
...
class HermiteEnve(Pulse):
    ...
    def __init__(self, duration, c_para = 0.667, T_para = 0.1667, mu = 0.5, amp=1.0, marker=0b00):
    ...
    def func(self, samples):
        ...
        samples = self.amp * (1-self.c_para*((samples-(self.peak))
                                        /self.T_width) **2) *np.exp( -( (samples-(self.peak))
                                        /self.T_width) **2 )
        return samples
...
class IQWaveform():
    ...
    def __init__(self, name, pulse_seq, offset=0, sampling=1.0E+09,
        ...
        name_i = name + '_I'
        name_q = name + '_Q'
        self.i = Waveform(name_i, pulse_seq, offset=offset, file_type=file_type, sampling=sampling)
        ...
        self.q = Waveform(name_q, pulse_seq, offset=offset, file_type=file_type, sampling=sampling)
