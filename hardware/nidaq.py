...
class CounterBoard:
    ...
    def __init__(
        self, CounterIn, CounterOut, TickSource, SettlingTime=2e-3, CountTime=8e-3
    ):
        self._CODevice = CounterOut
        self._CIDevice = CounterIn

        self._PulseTrain = (
            self._CODevice + "InternalOutput"
        )  # counter bins are triggered by CTR1
        ...
        if N < numpy.inf:
            CHK(
                dll.DAQmxCfgImplicitTiming(
                    self.COTask, DAQmx_Val_ContSamps, ctypes.c_ulonglong(N)
                )
            )
            CHK(
                dll.DAQmxCfgImplicitTiming(
                    self.CITask, DAQmx_Val_FiniteSamps, ctypes.c_ulonglong(N)
                )
            )
            ...
            CHK(
                dll.DAQmxSetReadOverWrite(
                    self.CITask, DAQmx_Val_DoNotOverwriteUnreadSamps
                )
            )
            ...
            CHK(
                dll.DAQmxCfgImplicitTiming(
                    self.COTask, DAQmx_Val_ContSamps, ctypes.c_ulonglong(BufferLength)
                )
            )
            CHK(
                dll.DAQmxCfgImplicitTiming(
                    self.CITask, DAQmx_Val_ContSamps, ctypes.c_ulonglong(BufferLength)
                )
            )
    ...
    def setTiming(self, SettlingTime, CountTime):
        if SettlingTime != self._SettlingTime or CountTime != self._CountTime:
            ...
            DutyCycle = CountTime * f
            CHK(
                dll.DAQmxSetCOPulseFreq(self.COTask, self._CODevice, ctypes.c_double(f))
            )
            CHK(
                dll.DAQmxSetCOPulseDutyCyc(
                    self.COTask, self._CODevice, ctypes.c_double(DutyCycle)
                )
            )
    ...
    def startCounter(self, SettlingTime, CountTime):
        if self.CountLength() != numpy.inf:
            self.setCountLength(
                numpy.inf, max(1000, self._CountAverageLength), self._CountAverageLength
            )
        self.setTiming(SettlingTime, CountTime)
        self.StartCI()
        self.StartCO()
...
class MultiBoard(CounterBoard):
    ...
    def __init__(
        self, CounterIn, CounterOut, TickSource, AOChannels, v_range=(0.0, 10.0)
    ):
...
class Scanner(MultiBoard):
    ...
    def scanLine(self, Line, SecondsPerPoint, return_speed=None):
        ...
        if (
            self.AOLength() != N
        ):  # set buffers of nidaq Tasks, data read buffer and timeout if needed
            self.setAOLength(N)
        ...
        if return_speed is not None:
            self.setTiming(
                SecondsPerPoint * 0.5 / return_speed,
                SecondsPerPoint * 0.5 / return_speed,
            )
    ...
    # we change the center point of the x axis to -300 mV, so the range is -1.3V to 0.7V.   Y [-1V,1V]   Z [0V, 10V]
    # the v_range [-10,10] in the custom.api file is used to give enough range for the AOChannel creation.
    # 20210313 JiahaoWU: Calibrate galvo center point to x_V = -200mV, y_V = -27mV
    def PosToVolt(self, r):
        ...
        v = self.vRange
        vr = v[1] - v[0]
        vx = v[0] + (r[0] - x[0]) / (x[1] - x[0]) * vr
        vy = v[0] + (r[1] - y[0]) / (y[1] - y[0]) * vr
        vz = 0 + (r[2] - z[0]) / (z[1] - z[0]) * 10
...
class SquareWave(object):
    ...
    def __init__(
        self, square_wave_device, length=100, seconds_per_point=1e-3, duty_cycle=0.5
    ):
        ...
        CHK(
            dll.DAQmxCreateCOPulseChanFreq(
                ...
                DAQmx_Val_Low,
                ctypes.c_double(0),
                ctypes.c_double(1.0 / seconds_per_point),
                ctypes.c_double(duty_cycle),
            )
        )
        CHK(
            dll.DAQmxCfgImplicitTiming(
                self._co_task, DAQmx_Val_FiniteSamps, ctypes.c_ulonglong(length)
            )
        )

    def setTiming(self, seconds_per_point, duty_cycle=0.5):
        CHK(
            dll.DAQmxSetCOPulseFreq(
                self._co_task,
                self._square_wave_device,
                ctypes.c_double(1.0 / seconds_per_point),
            )
        ...
        CHK(
            dll.DAQmxSetCOPulseDutyCyc(
                self._co_task, self._square_wave_device, ctypes.c_double(duty_cycle)
            )
        )
        self._seconds_per_point = seconds_per_point
        self._duty_cycle = duty_cycle
    ...
    def output(self):
        ...
        CHK(
            dll.DAQmxWaitUntilTaskDone(
                self._co_task,
                ctypes.c_double(4 * self._length * self._seconds_per_point),
            )
        )
...
class AnalogOutSyncCount:
    ...
    def __init__(
        self, ao_chan, co_dev, ci_dev, ci_port, ao_range=(-10, 10), duty_cycle=0.96
    ):
        ...
        CHK(
            dll.DAQmxCreateCIPulseWidthChan(
                ...
                ctypes.c_double(10000.0),  # expected max
                DAQmx_Val_Ticks,
                DAQmx_Val_Rising,
                "",
            )
    ...
    def configure(self, n_samples, seconds_per_point):
        ...
        if n_samples == 0:
            ...
            CHK(
                dll.DAQmxCfgImplicitTiming(
                    self.co_task, DAQmx_Val_ContSamps, ctypes.c_ulonglong(n_samples + 1)
                )
            )
            CHK(
                dll.DAQmxCfgImplicitTiming(
    ...
    def line(self, voltage, seconds_per_point):
        ...
        return ci_data[:-1] / (
            self.seconds_per_point * self.duty_cycle
        )  # -ci_data[:-1]
...
class AOTask(object):
    ...
    def __init__(
        self, Channels, N=numpy.inf, f=None, range=(-10, 10), write_timeout=1.0
    ):
...
class AITask(object):
    ...
    def __init__(self, Channels, N, f, read_timeout=1.0, range=(-10, 10)):
        ...
        self.timeout = ctypes.c_double(4.0 * N / f)
...
class PulseTrainCounter:
    ...
    def configure(
        self, SampleLength, SecondsPerPoint, DutyCycle=0.9, MaxCounts=1e7, RWTimeout=1.0
    ):
        ...
        # ctr2(CO channel) generates a continuous square wave with given duty cycle. This serves simultaneously
        # as sampling clock for AO (update DAC at falling edge), and as gate for counter (count between
        # rising and falling edge)
        CHK(
            dll.DAQmxCreateCOPulseChanFreq(
        ...
        # specify the time between two gates for the counting period(pulse train frequency generated by the CO channel)

        # ctr1 is used to count photons.(in order to avoid conflict, ctr3 (CI)instead. but the ticksouce is shared with ctr1(PFI3 ctr1 SRC))
        # Used to count ticks in N+1 gates
        CHK(
            dll.DAQmxCreateCIPulseWidthChan(
                self._CITask,
                self._CounterIn,
        ...
        # Meaning of arguments:  taskHandle, channel, *data
        CHK(
            dll.DAQmxSetCIPulseWidthTerm(
                self._CITask, self._CounterIn, self._CounterOut + "InternalOutput"
            )
        )  # terminal
        CHK(
            dll.DAQmxSetCICtrTimebaseSrc(
                self._CITask, self._CounterIn, self._TickSource
            )
        )

        CHK(
            dll.DAQmxCfgImplicitTiming(
                self._COTask, DAQmx_Val_ContSamps, ctypes.c_ulonglong(SampleLength)
            )
        )
        CHK(
            dll.DAQmxCfgImplicitTiming(
                self._CITask, DAQmx_Val_FiniteSamps, ctypes.c_ulonglong(SampleLength)
            )
        )
        ...
        CHK(
            dll.DAQmxSetReadOverWrite(self._CITask, DAQmx_Val_DoNotOverwriteUnreadSamps)
        )
    ...
    def run(self):
        ...
        CHK(
            dll.DAQmxWaitUntilTaskDone(self._CITask, ctypes.c_double(self._TaskTimeout))
        )
        CHK(
            dll.DAQmxReadCounterU32(
                self._CITask,
