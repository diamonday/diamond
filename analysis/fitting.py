...
def DoubleGaussian(a1, a2, x01, x02, w1, w2):
    ...
    return lambda x: a1 * np.exp(-0.5 * ((x - x01) / w1) ** 2) + a2 * np.exp(
        -0.5 * ((x - x02) / w2) ** 2
    )
...
setattr(
    DoubleGaussian,
    "formula",
    r"$f(c,a1, a2,x01, x02,w1,w2;x)=a_1\exp(-0.5((x-x_{01})/w_1)^2)+a_2\exp(-0.5((x-x_{02})/w_2)^2)$",
)
...
setattr(
    Antibunching,
    "formula",
    r"$g(\alpha,c,\tau,t_0;t)=c(1 - \alpha \exp(-(t-t_0)/\tau))$",
)
...
setattr(
    FCSTranslationRotation,
    "formula",
    r"$g(\alpha,\tau_R,\tau_T,N;t)=\frac{1 + \alpha \exp(-t/\tau_R)}{N (1 + t/\tau_T)}$",
)


def FCSTranslation(tau, N):
    """Fluorescence Correlation Spectroscopy. g(2) accounting for translational diffusion."""
    return lambda t: 1.0 / (N * (1 + t / tau))
...
def brot_transitions_upper(B, D, E, phase):
    return (
        lambda theta: 3.0 / 2.0 * B**2 / D * np.sin(theta + phase) ** 2
        + (
            B**2 * np.cos(theta + phase) ** 2
            + (E + B**2 / (2 * D) * np.sin(theta + phase) ** 2) ** 2
        )
        ** 0.5
        + D
    )
...
def grow(mask):
    ...
    return np.logical_or(
        np.logical_or(mask, np.append(mask[1:], False)), np.append(False, mask[:-1])
    )
...
def fit_multiple_lorentzians(x, y, number_of_lorentzians="auto", threshold=0.5):
    ...
    for left, right in left_and_right_edges:
        ...
        a = (
            y[i] * np.pi * g
        )  # height of local minimum in terms of Lorentzian parameter a
        p += [x0, g, a]
...
def CosinusNoOffsetPhase(a, T, phi):
    """Returns a Cosinus function without constant offset.

        f = a\cos(2\pi x/T + phi)
    ...
    phi   =  phase
    """
    return lambda x: a * np.cos(2 * np.pi * x / float(T) + phi)


setattr(CosinusNoOffset, "formula", r"$cos(a,T;x)=a\cos(2\pi x/T)$")


def CosinusNoOffsetPhaseEstimator(x, y):
    a, T = CosinusNoOffsetEstimator(x, y)
    phi = 0
    return a, T, phi
...
def fit_rabi_phase(x, y, s):
    ...
    p = fit(x, yp, CosinusNoOffsetPhase, CosinusNoOffsetPhaseEstimator)
    if p[0] < 0:
        p[0] = -p[0]
        p[2] = ((p[2] / p[1] + 0.5) % 1) * p[1]
        # p = fit(x, yp, CosinusNoOffset, p)
...
if __name__ == "__main__":
    ...
    d = pickle.load(open("point14_ESR_102000cts_cwODMR04_ODMR.pys", "rb"))
