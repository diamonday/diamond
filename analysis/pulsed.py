...
class PulsedAna(HasTraits, GetSetItemsMixin):
    ...
    position_normalize = Range(
        ...
        high=10000.0,
        value=-1.0,
        desc="position of normalization window relative to edge [ns]. If negative, no normalization is performed",
        label="pos. norm. [ns]",
        mode="text",
        auto_set=False,
        enter_set=True,
    ...
    run_sum = Range(
        ...
        high=1000,
        value=1,
        desc="running sum over n samples",
        label="running sum",
        mode="text",
        auto_set=False,
        enter_set=True,
    ...
    traits_view = View(
        VGroup(
            ...
            VSplit(
                ...
                Item(
                    "line_plot", show_label=False, width=500, height=300, resizable=True
                ),
                Item(
                    "pulse_plot",
                    show_label=False,
    ...
    def __init__(self, **kwargs):
        ...
        self.on_trait_change(
            self._update_matrix_index,
            "measurement.time_bins,measurement.n_laser",
            dispatch="ui",
        )
        self.on_trait_change(
            self._update_matrix_value, "measurement.count_data", dispatch="ui"
        )
        self.on_trait_change(
            self._update_pulse_index, "measurement.time_bins", dispatch="ui"
        )
    ...
    @on_trait_change(
        "measurement.count_data,integration_width,position_signal,position_normalize"
    )
    ...
    def _create_pulse_plot(self):
        pulse_data = ArrayPlotData(x=np.array((0.0, 0.1, 0.2)), y=np.array((0, 1, 2)))
...
class PulsedAnaTauRef(PulsedAnaTau):
    ...
    def _update_line_plot_value(self):
        ...
        self.line_data.set_data(
            "dark", np.mean(y[n + n_ref : n + 2 * n_ref]) * np.ones(n)
        )
...
class PulsedAnaDoubleRef(PulsedAna):
    ...
    def _update_line_plot_value(self):
        ...
        self.line_data.set_data(
            "bright", np.mean(y[2 * n : 2 * n + n_ref]) * np.ones(n)
        )
...
class FitAnaTau(PulsedAnaTau):
    ...
    def _create_line_plot(self):
        line_data = ArrayPlotData(
            index=np.array((0, 1)), spin_state=np.array((0, 0)), fit=np.array((0, 0))
        )
        plot = Plot(line_data, padding=8, padding_left=64, padding_bottom=36)
        plot.plot(("index", "spin_state"), color="blue", name="spin_state")
...
class RabiAna(FitAnaTau):
    ...
    contrast = Tuple(
        (np.nan, np.nan),
        editor=TextEditor(
            auto_set=False, enter_set=True, evaluate=float, format_str=" %.1f+-%.1f %%"
        ),
    )
    period = Tuple(
        (np.nan, np.nan),
        editor=TextEditor(
            auto_set=False, enter_set=True, evaluate=float, format_str=" %.2f+-%.2f"
        ),
    )
    q = Float(
        np.nan,
    ...
    # add fit results to the get_set_items
    get_set_items = FitAnaTau.get_set_items + [
        "contrast",
        "period",
        "t_pi2",
        "t_pi",
    ...
    def _update_fit(
        self, y
    ):  # ToDo: test and test whether 'y' can be changed to e.g. 'y'
    ...
    def _on_fit_result_change(self, new):
        if len(new) > 0 and new[0][0] is not np.NaN:
            self.line_data.set_data(
                "fit", fitting.Cosinus(*new[0])(self.measurement.tau)
            )
...
class PulseExtractAna(PulsedAnaTau):
    ...
    # add fit results to the get_set_items
    get_set_items = PulsedAnaTau.get_set_items + [
        "contrast",
        "period",
        "t_pi2",
        "t_pi",
    ...
    def _update_fit(self, y):
        ...
        self.line_data.set_data(
            "pulse_indices", np.hstack((pi2, pi, three_pi2, two_pi)) * 1e-3
        )
...
class PulseCalAna(PulsedAna):
    ...
    def _create_line_plot(self):
        ...
        plot.value_axis.title = "spin state"
        plot.overlays.insert(
            0, PlotLabel(text="", hjustify="left", vjustify="bottom", position=[70, 40])
        )
        self.line_data = line_data
        self.line_plot = plot
...
class PulsedFit(HasTraits, GetSetItemsMixin):
    ...
    plots = [
        {"data": ("pulse_number", "spin_state"), "color": "blue", "name": "pulsed"}
    ]
...
class FrequencyFit(PulsedFit):
    ...
    def update_plot_spin_state(self):
        ...
        if old_mesh is not None and len(old_mesh) != len(self.measurement.frequencies):
            self.line_data.set_data(
                "pulse_number", self.measurement.frequencies / 1.0e6
            )
        self.line_data.set_data("spin_state", self.spin_state)
...
class Frequency3pi2Fit(PulsedFit):
    ...
    # The list elements are dictionaries that are directly passed to the Plot.plot() command as keyword arguments through the **kwagrs expansion
    plots = [
        {"name": "nopi", "data": ("pulse_number", "spin_state_nopi"), "color": "blue"},
...
class PulsedFitErrorBars(PulsedFit):
    ...
    plots = [
        {"data": ("pulse_number", "spin_state"), "color": "blue", "name": "pulsed"},
        {
            "data": ("pulse_number", "spin_state_low"),
            "type": "scatter",
            "marker": "dot",
            "marker_size": 1.0,
            "marker_color": "black",
        ...
        {
            "data": ("pulse_number", "spin_state_high"),
            "type": "scatter",
            "marker": "dot",
            "marker_size": 1.0,
            "marker_color": "black",
    ...
    def __init__(self):
        ...
        self.on_trait_change(
            self.update_plot_error_bars, "spin_state_error", dispatch="ui"
        )

    def update_plot_error_bars(self):
        self.line_data.set_data(
            "spin_state_low", self.spin_state - self.spin_state_error
        )
        self.line_data.set_data(
            "spin_state_high", self.spin_state + self.spin_state_error
        )
...
class PulsedFitTau(PulsedFit):
    ...
    line_data = Instance(
        ArrayPlotData,
        factory=ArrayPlotData,
        kw={"tau": np.array((0, 1)), "spin_state": np.array((0, 0))},
    )
...
class DoubleFitTauRef(PulsedFit):
    ...
    plots = [
        {"data": ("tau", "first"), "color": "blue", "name": "first"},
...
class DoubleFitRef(PulsedFit):
    ...
    plots = [
        {"data": ("pulse_number", "first"), "color": "blue", "name": "first"},
    ...
    def update_plot_spin_state(self):
        ...
        n = len(spin_state) / 2 - n_ref
        if (
            old_mesh is not None and len(old_mesh) != n
        ):  # ToDo: there could be a tau mesh with length n
            self.line_data.set_data("pulse_number", np.arange(n))
        self.line_data.set_data("first", spin_state[:n])
        self.line_data.set_data("second", spin_state[n : -2 * n_ref])
        self.line_data.set_data(
            "bright", np.mean(self.spin_state[-2 * n_ref : -n_ref]) * np.ones(n)
        )  # ToDo: better use h_line
        self.line_data.set_data(
            "dark", np.mean(self.spin_state[-n_ref:]) * np.ones(n)
        )  # ToDo: better use h_line
...
class NMRFit(PulsedFit):
    ...
    freq = Float(0.0)  # Property( depends_on='fit_result', label='period' )
    linew = Float(0.0)
    contrast = Float(0.0)  # Property( depends_on='fit_result', label='contrast' )
    q = Float(0.0)  # Property( depends_on='fit_result', label='q' )

    def __init__(self):
        super(NMRFit, self).__init__()
        self.on_trait_change(self.update_fit, "spin_state", dispatch="ui")
        self.on_trait_change(
            self.update_plot_tau, "measurement.frequencies", dispatch="ui"
        )
    ...
    def update_fit(self):
        # try to compute new fit from spin_state and tau. If it fails, set result to NaNs
        try:
            fit_result = fitting.fit_multiple_lorentzian(
                self.measurement.frequencies, self.spin_state, 1
            )
        except:
            fit_result = (
                np.NaN * np.zeros(3),
    ...
    plots = [
        {"data": ("frequencies", "spin_state"), "color": "blue", "name": "NMR"},
    ...
    def update_plot_fit(self):
        if self.fit_result[0][0] is not np.NaN:
            self.line_data.set_data(
                "fit", fitting.NLorentzians(*self.fit_result)(self.frequencies)
            )
...
class RabiFit(PulsedFit):
    ...
    period = Tuple((0.0, 0.0))  # Property( depends_on='fit_result', label='period' )
    contrast = Tuple(
        (0.0, 0.0)
    )  # Property( depends_on='fit_result', label='contrast' )
    q = Float(0.0)  # Property( depends_on='fit_result', label='q' )
    t_pi2 = Tuple((0.0, 0.0))  # Property( depends_on='fit_result', label='pi/2' )
    t_pi = Tuple((0.0, 0.0))  # Property( depends_on='fit_result', label='pi' )
    t_3pi2 = Tuple((0.0, 0.0))  # Property( depends_on='fit_result', label='3pi/2' )
    ...
    def update_fit(self):
        ...
        try:
            fit_result = fitting.fit_rabi(
                self.measurement.tau, self.spin_state, self.spin_state_error
            )
        except:
            fit_result = (
                np.NaN * np.zeros(3),
    ...
    def update_plot_fit(self):
        if self.fit_result[0][0] is not np.NaN:
            self.line_data.set_data(
                "fit", fitting.Cosinus(*self.fit_result[0])(self.measurement.tau)
            )
...
class PulseExtraction(PulsedFit):
    ...
    def update_fit(self):
        ...
        # markers in the plot that show the result of the pulse extraction
        self.line_data.set_data(
            "pulse_indices", np.hstack((pi2, pi, three_pi2, two_pi))
        )
        self.line_data.set_data("pulse_values", np.hstack((y[f], y[p], y[r], y[tp])))
    ...
    plots = [
        {"data": ("tau", "spin_state"), "color": "blue", "name": "rabi"},
        {
            "data": ("pulse_indices", "pulse_values"),
            "type": "scatter",
            "marker": "circle",
            "color": "none",
            "outline_color": "red",
...
class ExponentialFit(PulsedFit):
    ...
    def _get_fit_parameters(self):
        try:
            return fitting.fit(
                self.measurement.tau,
                self.spin_state,
                fitting.ExponentialZero,
                fitting.ExponentialZeroEstimator,
            )
        except:
    ...
    def update_plot_fit(self):
        if self.fit_parameters[0] is not np.NaN:
            self.line_data.set_data(
                "fit",
                fitting.ExponentialZero(*self.fit_parameters)(self.measurement.tau),
            )

    traits_view = View(
        Tabbed(
            HGroup(Item("T1", style="readonly", width=-80), label="fit_parameter"),
            HGroup(
                Item("integration_width"),
                Item("position_signal"),
                Item("position_normalize"),
...
class HahnFit(PulsedFit):
    ...
    def _get_fit_result(self):
        try:
            return fitting.nonlinear_model(
                x,
                y,
                s,
                fitting.ExponentialPowerZero,
                fitting.ExponentialPowerZeroEstimator,
            )
        except:
            return (np.NaN * np.zeros(4), np.NaN * np.zeros((4, 4)), np.NaN, np.NaN)
    ...
    plots = [
        {"data": ("tau", "spin_state"), "color": "blue", "name": "hahn"},
    ...
    def update_plot_fit(self):
        if self.fit_result[0][0] is not np.NaN:
            self.line_data.set_data(
                "fit",
                fitting.ExponentialPowerZero(*self.fit_result[0])(self.measurement.tau),
            )
...
class DoubleFit(PulsedFit):
    ...
    plots = [
        {
            "data": ("pulse_number", "first_sequence"),
            "color": "blue",
            "name": "first sequence",
        },
        {
            "data": ("pulse_number", "second_sequence"),
            "color": "green",
...
class DoubleFitTau(DoubleFit):
    ...
    plots = [
        {"data": ("tau", "first_sequence"), "color": "blue", "name": "first sequence"},
        {
            "data": ("tau", "second_sequence"),
            "color": "green",
            "name": "second sequence",
        },
    ]
...
class Gaussian3pi2Fit(PulsedFit):
    ...
    def __init__(self):
        ...
        self.on_trait_change(
            self.update_plot_fit_3pi2, "fit_result_3pi2", dispatch="ui"
        )
    ...
    def _get_fit_result_pi2(self):
        ...
        try:
            return fitting.nonlinear_model(
                x, y, s, fitting.GaussianZero, fitting.GaussianZeroEstimator
            )
        except:
    ...
    def update_plot_fit_pi2(self):
        if self.fit_result_pi2[0][0] is not np.NaN:
            self.line_data.set_data(
                "fit_pi2",
                fitting.GaussianZero(*self.fit_result_pi2[0])(self.measurement.tau),
            )
    ...
    def update_plot_fit_3pi2(self):
        if self.fit_result_3pi2[0][0] is not np.NaN:
            self.line_data.set_data(
                "fit_3pi2",
                fitting.GaussianZero(*self.fit_result_3pi2[0])(self.measurement.tau),
            )
...
class Hahn3pi2Fit(PulsedFit):
    ...
    def update_plot_fit_pi2(self):
        if self.fit_result_pi2[0][0] is not np.NaN:
            self.line_data.set_data(
                "fit_pi2",
                fitting.ExponentialPowerZero(*self.fit_result_pi2[0])(
                    self.measurement.tau
                ),
            )
    ...
    def update_plot_fit_3pi2(self):
        if self.fit_result_3pi2[0][0] is not np.NaN:
            self.line_data.set_data(
                "fit_3pi2",
                fitting.ExponentialPowerZero(*self.fit_result_3pi2[0])(
                    self.measurement.tau
                ),
            )
...
class T1piFit(PulsedFit):
    ...
    def __init__(self):
        ...
        self.on_trait_change(self.update_plot_tau, "measurement.tau", dispatch="ui")
        self.on_trait_change(
            self.update_plot_fit_nopi, "fit_result_nopi", dispatch="ui"
        )
    ...
    def _get_fit_result_nopi(self):
        ...
        try:
            return fitting.nonlinear_model(
                x, y, s, fitting.ExponentialZero, fitting.ExponentialZeroEstimator
            )
        except:
            return (np.NaN * np.zeros(3), np.NaN * np.zeros((3, 3)), np.NaN, np.NaN)
    ...
    plots = [
        {"name": "data_nopi", "data": ("tau", "data_nopi"), "color": "blue"},
    ...
    def update_plot_fit_nopi(self):
        if self.fit_result_nopi[0][0] is not np.NaN:
            self.line_data.set_data(
                "fit_nopi",
                fitting.ExponentialZero(*self.fit_result_nopi[0])(self.measurement.tau),
            )
    ...
    def update_plot_fit_pi(self):
        if self.fit_result_pi[0][0] is not np.NaN:
            self.line_data.set_data(
                "fit_pi",
                fitting.ExponentialZero(*self.fit_result_pi[0])(self.measurement.tau),
            )
...
class PulsedAnalyzerHandler(GetSetItemsHandler):
    ...
    def new_rabi_2smiq_measurement(self, info):
        """info.object.measurement = mp.Rabi()"""
        info.object.measurement = newrabi.Rabi_2smiq()
        info.object.fit = RabiFit()

    def new_rabi_3smiq_measurement(self, info):
        """info.object.measurement = mp.Rabi()"""
        info.object.measurement = newrabi.Rabi_3smiq()
        info.object.fit = RabiFit()
    ...
    def new_fid3pi2_200laser_measurement(self, info):
        info.object.measurement = mp.FID3pi2_200laser()

    def new_count_decay_measurement(self, info):
        info.object.measurement = mp.Count_decay()
    ...
    def new_CPMG3pi2DD_measurement(self, info):
        info.object.measurement = mp.CPMG3pi2_DD()

    def new_XY83pi2DD_measurement(self, info):
        info.object.measurement = mp.XY83pi2_DD()

    def new_MWSweep_measurement(self, info):
        info.object.measurement = mp.MW_Sweep()

    def new_PulPolSweep_measurement(self, info):
        info.object.measurement = mp.PulPol_Sweep()
...
class PulsedAnalyzer(HasTraits, GetSetItemsMixin):
    ...
    def __init__(self):
        ...
        self.on_trait_change(
            self.refresh_matrix_axis,
            "measurement.time_bins,measurement.n_laser",
            dispatch="ui",
        )
        self.on_trait_change(
            self.refresh_matrix, "measurement.count_data", dispatch="ui"
        )
        self.on_trait_change(self.refresh_pulse, "fit.pulse", dispatch="ui")
        self.on_trait_change(
            self.refresh_time_bins, "measurement.time_bins", dispatch="ui"
        )
    ...
    def _matrix_plot_data_default(self):
        return ArrayPlotData(
            image=np.zeros((self.measurement.n_laser, self.measurement.n_bins))
        )
    ...
    def refresh_flank(self):
        self.pulse_plot.components[1].index.set_data(
            np.array((self.fit.flank, self.fit.flank))
        )
    ...
    def _fit_changed(self, name, old, new):
        ...
        for key in list(plot.plots.keys()):
            plot.delplot(key)
        # set new data source
    ...
    traits_view = View(
        ...
        menubar=MenuBar(
            ...
            Menu(
                ...
                Action(action="new_rabi_2smiq_measurement", name="Rabi 2 smiq"),
                Action(action="new_rabi_3smiq_measurement", name="Rabi 3 smiq"),
                ...
                # Action(action='new_fid3pi2_200laser_measurement', name='FID3pi2_200laser'),
                Action(action="new_count_decay_measurement", name="count decay"),
                ...
                Action(
                    action="new_hartmannhahn3pi2_measurement", name="HartmannHahn3pi2"
                ),
                ...
                Action(
                    action="new_HardDQTFIDTauMod_measurement", name="HardDQTFIDTauMod"
                ),
                ...
                Action(action="new_CPMG3pi2DD_measurement", name="CPMG3pi2DD"),
                Action(action="new_XY83pi2DD_measurement", name="XY83pi2DD"),
                Action(action="new_PulPolSweep_measurement", name="PulPolSweep"),
                Action(action="new_MWSweep_measurement", name="MWSweep"),
