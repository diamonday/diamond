...
class Keithley2612(HasTraits):
    ...
    current = Range(low = 0, high=15000, value = 0, desc='current[mA]', label='current[mA]', auto_set=False,
    ...
    def _Measure_fired(self):
        self.ReadingV = np.float(self.k2612.ask("print(smub.measure.v())")) * 1000.
        self.ReadingI = np.float(self.k2612.ask("print(smub.measure.i())")) * 1000.
    ...
    traits_view = View(VGroup(HGroup(Item('current', width=-500)),
                              ...
                              HGroup(Item('TTLOn'),    
                                     Item('TTLOff')) ),
                       title="Keithley 2612 console(current mode)", width=600, height=150, buttons=[], resizable=True)
