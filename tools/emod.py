...
class Job(HasTraits):
    ...
    def start(self):
        ...
        self.thread = StoppableThread(
            target=self._run, name=self.__class__.__name__ + timestamp()
        )
        self.thread.start()
        ...
        try:
            if job is self.running:
                ...
                if not job in self.queue:
                    ...
                    logging.debug("Job " + str(job) + " removed.")
                    job.state = (
                        "idle"  # ToDo: improve handling of state. Move handling to Job?
                    )
        finally:
            self.lock.release()
...
class ManagedJob(Job):
    ...
    submit_button = Button(
        label="submit", desc="Submit the measurement to the job manager."
    )
    remove_button = Button(
        label="remove",
        desc="Remove the measurement from the job manager. Stop it if necessary.",
    )
    ...
    def submit(self):
        ...
        JobManager().submit(
            self
        )  # we just submit again and again. The JobManager takes care that duplicate submissions are ignored

    def remove(self):
        """Remove the job from the JobManager. Stop job if necessary."""
        JobManager().remove(
            self
        )  # we just try to remove. The JobManager takes care that unknown jobs are ignored
