...
class AutoODMR(ManagedJob, GetSetItemsMixin):
    ...
    def focus(self):
        ...
        if (
            (not np.isnan(self.odmr.fit_frequencies[0]))
            and (not np.isnan(self.odmr.fit_line_width))
            and (not np.isnan(self.odmr.fit_contrast))
        ):
            ...
            logging.getLogger().info(
                "Auto ODMR: %.2f, %.2f, %.2f"
                % (self.fit_frequency, self.fit_line_width, self.fit_contrast)
            )
        else:
