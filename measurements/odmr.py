...
from traitsui.api import View, Item, HGroup, VGroup, VSplit, TextEditor
...
class ODMR(ManagedJob, GetSetItemsMixin):
    ...
    power = Range(low= -100., high=25., value= -27, desc='Power [dBm]', label='Power [dBm]', mode='text', auto_set=False, enter_set=True)
    ...
    frequency_end = Range(low=1, high=20e9, value=2.92e9, desc='Stop Frequency [Hz]', label='End [Hz]', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    ...
    number_of_resonances = Trait(2, String('auto', auto_set=False, enter_set=True), Int(10000., desc='Number of Lorentzians used in fit', label='N', auto_set=False, enter_set=True))
    threshold = Range(low= -99, high=99., value= -85., desc='Threshold for detection of resonances [%]. The sign of the threshold specifies whether the resonances are negative or positive.', label='threshold [%]', mode='text', auto_set=False, enter_set=True)
    ...
    def _run(self):
        ...
        try:
            ...
            if self.pulsed:
                ...
                ha.Microwave().setPower(self.power)   # here output on
