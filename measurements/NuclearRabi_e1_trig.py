...
class NuclearRabi_e1_trig( Pulsed ):
    ...
    get_set_items = Pulsed.get_set_items + [ 'freq_awg','freq_awg_rf', 'amp1','amp1_rf', 'show_fit', 'fit_parameters', 'rabi_frequency', 'rabi_period',
        'rabi_offset', 'rabi_contrast', 'pi2', 'pi', 'pi32', 'phase_sin', 'mw_pi'
    ]
    
    def __init__(self, pulse_generator, time_tagger, microwave, awg, awg_rf, **kwargs):
        self.dual = False
        super(NuclearRabi_e1_trig, self).__init__(pulse_generator, time_tagger, microwave, awg, awg_rf, **kwargs)
        self.show_fit = True
    ...
    def generate_sequence(self):
        ...
        for t in tau:
            ...
            sequence.append(([ ], trig_delay + mw_pi + t)) # since trig_interval_rf is around 523ns and trig_delay is 50ns
            ...
            #if t == tau[0]:
            #    sequence.append((['ch2','awg'], trig_interval))
            #else:
            #    sequence.append((['awg'], trig_interval))
            '''
            sequence.append((['awg'], trig_interval))
            sequence.append(([], trig_delay + mw_pi)) #1/5
            #sequence.append(([], trig_delay + 2 * mw_pi))
            sequence.append((['awg_rf'], trig_interval_rf))
            sequence.append(([], trig_delay_rf + t))
            sequence.append((['aom', 'laser', 'ch3'], laser))
    ...
    def compile_waveforms(self):
        ...
        mw2 = Sin(2 * mw_pi, freq=f1, amp=self.amp1, phase=phase) #2/5
        zero = Idle(1)
        ...
        pulse_sequence2 = [zero, mw2, zero] #3/5
        ...
        for i,t in enumerate(self.tau):
            seq.append(waves, wait=True) #5/5
    ...
    traits_view = View(
        ...
        title='NuclearRabi e1 trig (AWG610)',
        buttons=[],
        resizable=True,
        width =1400,
        height=800,
