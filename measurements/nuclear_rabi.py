...
class NuclearRabi(Pulsed):
    ...
    switch = Enum(
        ...
        "rf_2",
        "mw_b",
        desc="switch to use for different RF source",
        label="switch",
    )
    rf_frequency = Range(
        low=1,
    ...
    do_0n1 = Bool(
        False,
        desc="measurement -1,0 to -1,-1 at ELAC",
        label="do 0-1",
    )
    rf0p1_frequency = Range(
        low=1,
        high=200e6,
        value=2.780e6,
        desc="RF frequency",
        label="RF0p1 frequency [Hz]",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    ...
    traits_view = View(
        VGroup(
            ...
            Tabbed(
                VGroup(
                    HGroup(
                        Item("mw_frequency", width=-80, enabled_when='state == "idle"'),
                    ...
                    HGroup(
                        Item("do_0n1", width=-80, enabled_when='state == "idle"'),
                        Item(
                            "rf0p1_power",
                            width=-80,
                            enabled_when='state == "idle" and do_0n1 == True',
                        ),
                        Item(
                            "rf0p1_frequency",
                            width=-80,
