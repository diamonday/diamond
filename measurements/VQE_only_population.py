...
class VQE(Pulsed):
    ...
    def generate_sequence(self):
        ...
        if self.check_state:
            i_range = 1
            max_time_rottome = (
                t_2pi_rot_e / 2 + t_rot + t_2pi_mw_13 / 2 + t_2pi_rf_34 / 2
            )
        else:
            i_range = 5
            max_time_rottome = (
        ...
        max_time_norm_pol = (
            t_2pi_rf_34 / 2 + t_2pi_rot_e / 2 + t_2pi_rf_34 / 2 + t_2pi_mw_24 / 2
        )  # the third of pol_sequences

        max_time = max(max_time_rottome, max_time_norm_pol)
        ...
        for i in range(i_range):  # 5 for self and 2 gradient*2
            ...
            for j in range(4):  # 4 fluorescence rates of the different levels
                if j == 0:
                    sequence.append(
                        ([], max_time - 100 - norm_sequences[j][1])
            ...
            for k in range(4):  # 4 to do tomo diag terms
                sequence.append(
                    ([], max_time - rot_sequences[i][1] - tomo_sequences[k][1])
                )
                sequence.append((["awg_rf"], trig_interval_rf))
                sequence.append(([], trig_delay_rf - trig_delay - trig_interval))
                sequence.append((["awg"], trig_interval))
    ...
    def VQE_wfm(self, i, j):
        ...
        t_pi_combined * self.sampling_rf / 1e9
