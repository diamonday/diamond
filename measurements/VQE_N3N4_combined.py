...
class VQE_N3N4(Pulsed):
    ...
    def generate_sequence(self):
        ...
        rot_sequences.append(([], t_pi_combined + t_rot))
        rot_sequences.append(([], t_pi_combined + t_rot - t_2pi_rf_34 / 4))  # for ny
        rot_sequences.append(([], t_pi_combined + t_rot + t_2pi_rf_34 / 4))
        rot_sequences.append(([], t_pi_combined + t_rot - t_2pi_rf_34 / 4))  # for nx
        rot_sequences.append(([], t_pi_combined + t_rot + t_2pi_rf_34 / 4))

        # 4 kinds of sequence for normlize N1 N3 N4 N2
        norm_sequences = []
        norm_sequences.append(([], 0))  # N1
        norm_sequences.append(([], t_pi_combined))  # N3
        norm_sequences.append(([], t_pi_combined + t_2pi_rf_34 / 2))  # N4
        norm_sequences.append(
            ([], t_pi_combined + t_2pi_rf_34 / 2 + t_pi_combined)
        )  # N2

        # polarization rates
        pol_sequences = []
        pol_sequences.append(
            ([], t_2pi_rf_34 / 2 + t_pi_combined)
        )  # first pi_34 then Big Pi, for e
        pol_sequences.append(([], t_2pi_mw_24 / 2))  # pi_24 for n
        pol_sequences.append(
            ([], t_2pi_rf_34 / 2 + t_pi_combined + t_2pi_rf_34 / 2 + t_2pi_mw_24 / 2)
        )  # pi_34, Big Pi, pi_34, pi_24, for n
        pol_sequences.append(
            ([], t_2pi_rf_34 / 2 + t_pi_combined + t_2pi_rf_34 / 2)
        )  # pi_34, Big Pi, pi_34, for n
        ...
        test_sequences.append(
            ([], t_2pi_rf_12 / 2 + t_2pi_mw_24 / 2)
        )  # a different way to transfer population to N4
        ...
        sequence = [
            ...
            (
                ["aom"],
                (576 + 512) * 1e9 / self.sampling_rf + 450,
            ),  # to be the same with other laser
            ([], wait),
        ]
    ...
    def VQE_wfm(self, i, j):
        ...
        self.t_2pi_rot_e * self.sampling_rf / 1e9
        ...
        pi_34_rf_detune = [
            Sin(t_2pi_rf_34 / 2, freq=freq_rf_34_detune, amp=self.amp_rf_34, phase=0)
        ]
        ...
        if j == 0:
            ...
            # rf
            if int(t_pi_combined_rft) < 512:
                carry = Idle(0)
                carry.duration = int(t_pi_combined_rft)
            ...
            if int(t_2pi_rf_34_mwt / 2) < stub:
                ...
                pulse_sequence = (
                    [zero] + Pi_all_combined + [carry] + Pi_all_combined + [zero]
                )
                name = "VQE_%02i_%02i" % (i, j)
                sub = IQWaveform(
                    name, pulse_sequence, offset=0, file_type=0, sampling=self.sampling
            ...
            if int(t_pi_combined_rft) < 512:
                ...
                print("t_pi_combined_rft too long!")
                sub_rf = Sequence("VQE_%02i_%02i_rf" % (i, j))
            ...
            # rf
            carry_rf = Idle(0)
            carry_rf.duration = int(t_pi_combined_rft)
            ...
            if int(t_2pi_rf_34_mwt / 2) < 256:  # usually this won't happen
                carry = Idle(0)
                carry.duration = int(t_2pi_rf_34_mwt / 2)
                pulse_sequence = [carry] + Pi_all_combined + [carry] + Pi_24 + [zero]
                name = "VQE_%02i_%02i" % (i, j)
                sub = IQWaveform(
                    name, pulse_sequence, offset=0, file_type=0, sampling=self.sampling
                )
                ...
                pulse_sequence1 = [carry1] + Pi_all_combined
                wfm1 = IQWaveform(
                    "VQE_%02i_%02i_1" % (i, j),
                    pulse_sequence1,
                    t_offset1,
            ...
            if int(t_2pi_rot_e / 2 + rot_rf_mwt) < 256:
                ...
                pulse_sequence = [zero] + Pi_all_combined + [carry] + Pi_13 + [zero]
                name = "VQE_%02i_%02i" % (i, j)
                sub = IQWaveform(
                    name, pulse_sequence, offset=0, file_type=0, sampling=self.sampling
                )
                ...
                sub = Sequence("VQE_%02i_%02i" % (i, j))
                pulse_sequence1 = [zero] + Pi_all_combined
            ...
            if int(t_2pi_rot_e / 2 + rot_rf_mwt) < 256:
                ...
                pulse_sequence = [zero] + Pi_all_combined + [carry] + Pi_24 + [zero]
                name = "VQE_%02i_%02i" % (i, j)
                sub = IQWaveform(
                    name, pulse_sequence, offset=0, file_type=0, sampling=self.sampling
                )
            ...
            # rf
            carry1_rf = Idle(0)
            carry1_rf.duration = int(t_pi_combined_rft)
            carry2_rf = Idle(0)
    ...
    def _create_line_plot(self):
        ...
        self.index_range = DataRange1D(
            self.x_data, low_setting=-0.5, high_setting=3.5, stretch_data=False
        )
