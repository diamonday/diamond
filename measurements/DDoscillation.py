...
class DDosc(measurePulsed, ManagedJob, GetSetItemsMixin):
    ...
    power = Range(low= -100., high=16., value= -10, desc='power of microwave', label='power[dBm]', mode='text', auto_set=False, enter_set=True)


    t_2pi_x = Range(low=1., high=100000., value=400., desc='rabi period 2pi pulse length (x)', label='2pi x [ns]', mode='text', auto_set=False, enter_set=True)
    t_2pi_y = Range(low=1., high=100000., value=400., desc='rabi period 2pi pulse length (y, 90 degree)', label='2pi y[ns]', mode='text', auto_set=False, enter_set=True)

    typeDD = Enum("CPMG", "XY4", "PulPol", desc='type of DD', label='DD type')    

    tau_DD = Range(low=1.5, high=1.0e+6, value=2.0e+3, desc='tau ',label='tau DD[ns]', mode='text',auto_set=False, enter_set=True)
    N_start = Range(low=0, high=1000, value=1, desc='starting number of pi pulses of transition a', label='N start', mode='text', auto_set=False, enter_set=True)
    N_end = Range(low=1, high=1000, value=10, desc='ending number of pi pulses of transition a', label='N end', mode='text', auto_set=False, enter_set=True)
    delta_N=Range(low=1, high=100, value=1, desc='the step in the N sequence', label='delta N', mode='text', auto_set=False, enter_set=True)
    N_ref = Range(low=1, high=1000, value=1, desc='number of reference pulse', label='N_ref', mode='text', auto_set=False, enter_set=True)

    N_seq = Array(value=np.array((0., 1, 2, 3)))
    repeat = Range(low=0, high=100, value=0, desc='repetition of the pulse sequence each run', label='repeat', mode='text', auto_set=False, enter_set=True)
    ...
    def get_DDsequence(self, N):
        ...
        if DDtype == "CPMG":
            t_pi2_x = t_2pi_x/4.0
            t_pi_y = t_2pi_y/2.0
            # tDD = tau_DD-t_pi_y-t_pi2_x/float(N)/4.0
            tDD = tau_DD-t_pi_y
            if tDD < 0:
                tDD = 0
            else:
            ...
            if N == 0:
                ...
                DDseq = [ (['mw_x'], t_pi2_x), ([], tDD/2.0)]
                DDseq += [ (['mw_y'], t_pi_y), ([], tDD)]*int(N-1)
                DDseq += [(['mw_y'], t_pi_y), ([], tDD/2.0), (['mw_x'], t_pi2_x)]
        elif DDtype == "XY4":
            t_pi2_x = t_2pi_x/4.0
            t_pi_x = t_2pi_x/2.0 
            t_pi_y = t_2pi_y/2.0
            # tDD = tau_DD-(t_pi_y+t_pi_x)/2.0-t_pi2_x/float(N)
            tDD = tau_DD-(t_pi_y+t_pi_x)/2.0
            if tDD < 0:
                tDD = 0
            else:
                tDD = tDD
            if N == 0:
                ...
                DDseq = [ (['mw_x'], t_pi2_x), ([], tDD/2.0)]
                DDseq += [ (['mw_y'], t_pi_y), ([], tDD), (['mw_x'], t_pi_x), ([], tDD),
                           (['mw_x'], t_pi_x), ([], tDD), (['mw_y'], t_pi_y), ([], tDD),]*int(N-1)
                DDseq += [ (['mw_y'], t_pi_y), ([], tDD), (['mw_x'], t_pi_x), ([], tDD),
                           (['mw_x'], t_pi_x), ([], tDD), (['mw_y'], t_pi_y), ([], tDD/2.0)]
                DDseq += [(['mw_x'], t_pi2_x)]
        elif DDtype == "PulPol":
            t_pi2_x = t_2pi_x/4.0
            t_pi2_y = t_2pi_y/4.0
            ...
            DDseq = [ (['mw_x'], t_pi2_x), ([], tDD/4.0), (['mw_y'], t_pi_y), ([], tDD/4.0), (['mw_x'], t_pi2_x),
                        (['mw_y'], t_pi2_y), ([], tDD/4.0), (['mw_x'], t_pi_x), ([], tDD/4.0), (['mw_y'], t_pi2_y)]*int(N)
        else:
            DDseq = []
        return DDseq
    ...
    def update_fitData(self):
        if self.perform_fit:
            ...
            self.fitData = "\nperiod: {}+-{}".format(round(para[1],1), round(para_dev[1], 3))
            self.para_fit = para
    ...
    get_set_items = measurePulsed.get_set_items + [
                    'laserTime', 'wait', 'frequency', 'power',  't_2pi_x', 't_2pi_y','tau_DD', 'typeDD',
                    'N_start', 'N_end', 'delta_N', 'N_ref', 'N_seq',
