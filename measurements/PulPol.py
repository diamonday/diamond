...
class PulPolHandler(GetSetItemsHandler):
...
class PulPol(measurePulsed, ManagedJob, GetSetItemsMixin):
    """
    +======================================================================================================+                                                                                    |
    ...
    repetition_seq = Array(value=np.array((0.0, 0.0)))
    # for depolarizing/mixing nuclear spin
    power_rf = Range(
        low=-100.0,
        high=16.0,
        value=-10,
        desc="power of radiowave",
        label="power rf[dBm]",
        mode="text",
        auto_set=False,
        enter_set=True,
    ...
    def generate_sequence(self):
        ...
        tDD = tau_DD - (t_pi_x + t_pi_y) - (t_pi2_y + t_pi2_x) * 2
        if tDD < 0:
            tDD = 0
        else:
            tDD = tDD
        ...
        # mixSeq = [(['mw_x', 'aom'], t_mix/40000.0), (['aom'], t_mix/4000.0), (['mw_x', 'aom'], t_mix/40000.0), (['aom'], t_mix/4000.0)]*2000 # for mixing nspin
        # mixSeq = ([ (['aom'], laserTime_pp), ([], wait)] + PI2_X  + [([], wait_pp)] + DDseq)*int(repetition)
        ...
        sequence += (DDseq + [(["laser", "aom"], laserTime_pp), ([], wait_pp)]) * int(
            repetition
        )
        sequence += [([], 150000)]
        ...
        sequence += N_ref * (
            [(["mw_x"], t_pi_ref)] + [(["laser", "aom"], laserTime), ([], wait)]
        )  # dark
    ...
    def update_fitData(self):
        if self.perform_fit:
            ...
            fitoutput = curve_fit(
                ExponentialZero, repetition_seq, signal, parameters_guess
            )
                ...
                VGroup(
                    HGroup(
                        ...
                        VGroup(
                            ...
                            Item(
                                "repetition", width=-80, enabled_when='state != "run"'
                            ),
                            Item(
                                "laserTime_pp", width=-80, enabled_when='state != "run"'
                            ),
                            Item("wait_pp", width=-80, enabled_when='state != "run"'),
                            Item(
                                "frequency_rf", width=-80, enabled_when='state != "run"'
                            ),
