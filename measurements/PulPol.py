...
class PulPol(measurePulsed, ManagedJob, GetSetItemsMixin):
    ...
    frequency = Range(low=1, high=20e9, value=2.8705e9, desc='microwave frequency', label='frequency mw[Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    ...
    def generate_sequence(self):
        ...
        sequence += (DDseq + [ (['laser', 'aom'], laserTime_pp), ([], wait_pp)])*int(repetition)
        sequence += [([], 150000)]

        #t_pi_ref = t_pi_x*(int(N_DD)*2 - 1)
        t_pi_ref = t_pi_x
        sequence += N_ref * ([ (['laser', 'aom'], laserTime), ([], wait)]) #bright
        sequence += N_ref * ([ (['mw_x'], t_pi_ref)] + [(['laser', 'aom'], laserTime), ([], wait)]) #dark
    ...
    get_set_items = measurePulsed.get_set_items + [
                    'laserTime', 'wait', 'frequency', 'power_mw',  't_2pi_x', 't_2pi_y','tau_DD', 
