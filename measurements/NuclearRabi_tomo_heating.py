...
class NuclearRabi_smiq_Nnorm(Pulsed):
    ...
    """N2"""
    freq_awg_rf_N2 = Range(
        low=1,
        high=2.6e9,
        value=5.1e6,
        desc="AWG_rf frequency N2",
        label="freq N2[Hz]",
        mode="text",
        auto_set=False,
        enter_set=True,
    ...
    mw_pi_rf_N2 = Range(
        ...
        high=10000000.0,
        value=100000,
        desc="length of rf pi pulse for N2 [ns]",
        label="mw_pi_rf_N2 [ns]",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    ...
    def generate_sequence(self):
        ...
        sequence = [
            ...
            (
                [],
                max_time + hard_pi_t + mw_pi + mw_pi_rf - 100,
        ...
        sequence.append(
            ([], max_time + hard_pi_t + mw_pi + mw_pi_rf)
        ...
        sequence.append(([], max_time + hard_pi_t + mw_pi + mw_pi_rf - mw_pi_rf_N2))
        sequence.append((["laser", "aom"], laser))
        sequence.append(
            (["aom"], (576 + 512) * 1e9 / self.sampling_rf + 450)
        )  # for trigger dead time of awg610(for awg520, we can ignore it if we set the sampling rate as 1e9)
        ...
        sequence.append(([], trig_delay_rf))
        sequence += hard_pi_seq
        sequence.append(([], max_time + mw_pi + mw_pi_rf))
        sequence.append((["laser", "aom"], laser))
        sequence.append(
            (["aom"], (576 + 512) * 1e9 / self.sampling_rf + 450)
        )  # for trigger dead time of awg610(for awg520, we can ignore it if we set the sampling rate as 1e9)
    ...
    def compile_waveforms(self):
        ...
        mw_rf_pi12 = Sin(
            mw_pi_rf_N2_rft, freq=f1_rf_N2, amp=self.amp1_rf, phase=phase_rf
        )
        ...
        # for nothing
        heating_time = max_time + hard_pi_t + mw_pi + mw_pi_rf - 100
        ...
        # for N1
        heating_time = max_time + hard_pi_t + mw_pi + mw_pi_rf
        rf_heating.duration = int(int(heating_time / T_heating) * T_heating)
        pulse_sequence_rf = [zero_rf] + [rf_heating] + [zero_rf]
        name_rf = "NRABI_N1"
        wave_rf = Waveform610(
        ...
        heating_time = max_time + hard_pi_t + mw_pi + mw_pi_rf - mw_pi_rf_N2
        rf_heating.duration = int(int(heating_time / T_heating) * T_heating)
        ...
        # for N3
        heating_time = max_time + mw_pi + mw_pi_rf
        rf_heating.duration = int(int(heating_time / T_heating) * T_heating)
        ...
        for i, t in enumerate(self.tau):
            ...
            rf_heating.duration = int(int(heating_time / T_heating) * T_heating)
            pulse_sequence_rf = (
                [carry] + [mw_rf] + [carry_pi13] + [rf_heating] + [zero_rf]
            )
            ...
            rf_heating.duration = int(int(heating_time / T_heating) * T_heating)
            pulse_sequence_rf = [carry] + [mw_rf] + [rf_heating] + [zero_rf]
            ...
            "Tomo Pi"
            heating_time = max_time - t - mw_pi_2 + mw_pi + mw_pi_rf
            rf_heating.duration = int(int(heating_time / T_heating) * T_heating)
            pulse_sequence_rf = (
                [carry] + [mw_rf] + [carry_pi24] + [rf_heating] + [zero_rf]
            )
