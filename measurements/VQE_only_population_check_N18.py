...
class VQE_test(Pulsed):
    ...
    def generate_sequence(self):
        ...
        test_sequences.append(
            ([], t_2pi_rot_e / 2 + t_2pi_rf_34 / 2 + t_2pi_rot_e / 2)
        )  # wait for t_2pi_rot_e/2 + t_2pi_rf_34/2 + t_2pi_rot_e/2 to check if the time cause the N1 > N2 or it is just the case
        test_sequences.append(
            ([], t_2pi_rot_e / 2 + t_2pi_rf_34 / 2 + t_2pi_rot_e / 2)
        )  # wait for t_2pi_rot_e/2 + t_2pi_rf_34/2 + t_2pi_rot_e/2 to check if the time cause the N1 > N2 or it is just the case
        test_sequences.append(
        ...
        test_sequences.append(
            ([], t_2pi_rot_e / 2 + t_2pi_rf_34 + t_2pi_rot_e / 2)
        )  # wait for t_2pi_rot_e/2 + t_2pi_rf_34/2 + t_2pi_rot_e/2 to check if the time cause the N1 > N2 or it is just the case
        test_sequences.append(
            ([], t_2pi_rf_12 / 2)
        )  # wait for t_2pi_rot_e/2 + t_2pi_rf_34/2 + t_2pi_rot_e/2 to check if the time cause the N1 > N2 or it is just the case
        ...
        for i in range(i_range):  # 5 for self and 2 gradient*2
            ...
            for k in range(4):  # 4 to do tomo diag terms
                ...
                sequence.append(([], wait))
            for m in range(6):  # to do some test
                sequence.append((["awg_rf"], trig_interval_rf))
                sequence.append(([], trig_delay_rf - trig_delay - trig_interval))
                sequence.append((["awg"], trig_interval))
                sequence.append(
    ...
    def VQE_wfm(self, i, j):
        ...
        # return sub, sub_waves, sub_rf, sub_waves_rf
            ...
            j == 14
        ):  # check Npi_pi34_pi(N_2): with a pi_34_rf_detune with freq 2.0 MHz
            # mw
            pulse_sequence = [zero] + Pi_all_x + [zero]
            wfm1 = IQWaveform(
                "VQE_%02i_%02i_1" % (i, j),
            ...
            j == 15
        ):  # check Npi_pi34_pi(N_2): with a Big Pi(Pi_all_combined) consisting of pi_13 + pi_24
            # mw
            ...
            if int(t_2pi_mw_13_rft / 2 + t_2pi_mw_24_rft / 2) < 512:
                ...
                carry = Idle(0)
                n, carry.duration = (
                    int(t_2pi_mw_13_rft / 2 + t_2pi_mw_24_rft / 2) / 512,
                    int(t_2pi_mw_13_rft / 2 + t_2pi_mw_24_rft / 2) % 512,
                )
    ...
    def _create_line_plot(self):
        ...
        self.index_range = DataRange1D(
            self.x_data, low_setting="auto", high_setting="auto", stretch_data=False
        )
