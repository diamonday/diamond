...
from chaco.api import Spectral, LinearMapper, ColorBar
...
class SNSCXYn2d(measurePulsed, ManagedJob, GetSetItemsMixin):
    ...
    scatterPlot_t1_start_Data = Instance(
        ArrayPlotData
    )  # plot data for symmetric signal of t2 at fixed t1, t1 is the start of tau_f sequence
    scatterPlot_t1_end_Data = Instance(
        ArrayPlotData
    )  # plot data for symmetric signal of t2 at fixed t1, t1 is the end of tau_f sequence
    scatterPlot_t2_start_Data = Instance(
        ArrayPlotData
    )  # plot data for symmetric signal of t1 at fixed t2, t2 is the start of tau_f sequence
    scatterPlot_t2_end_Data = Instance(
        ArrayPlotData
    )  # plot data for symmetric signal of t1 at fixed t2, t2 is the end of tau_f sequence
    scatterPlot_t1_start = Instance(
        Plot
    )  # scatter plot for symmetric signal of t2 at fixed t1, t1 is the start of tau_f sequence
    scatterPlot_t1_end = Instance(
        Plot
    )  # scatter plot for symmetric signal of t2 at fixed t1, t1 is the end of tau_f sequence
    scatterPlot_t2_start = Instance(
        Plot
    )  # scatter plot for symmetric signal of t1 at fixed t2, t2 is the start of tau_f sequence
    scatterPlot_t2_end = Instance(
        Plot
    )  # scatter plot for symmetric signal of t1 at fixed t2, t2 is the end of tau_f sequence
    ...
    def __init__(self):
        ...
        self.sync_trait(
            "thresh_high",
            self.correlation_spectrum_S.plots["image"][0].value_range,
            "high_setting",
        )
        self.sync_trait(
            "thresh_low",
        ...
        self.on_trait_change(
            self.correlation_spectrum_S.plots["image"][0].request_redraw,
            "thresh_high",
            dispatch="ui",
        )
        self.on_trait_change(
    ...
    def update_FTspectrum(self):
        ...
        for (a, b), element in np.ndenumerate(y):
            ...
            a2 = b1
            if (
                (a2 >= 0 and a2 <= (n_tau_f - 1))
                and (b2 >= 0 and b2 <= (2 * n_tau_f - 2))
            ) and (y2[a2, b2] == 0):
                y2[a2, b2] = element
    ...
    def _create_scatterPlot_t1_start(self):
        ...
        plot = Plot(
            self.scatterPlot_t1_start_Data,
            width=300,
            height=180,
    ...
    def _create_scatterPlot_t2_start(self):
        ...
        plot = Plot(
            self.scatterPlot_t2_start_Data,
            width=300,
            height=180,
            aspect_ratio=5.0 / 3.0,
            resizable="hv",
    ...
    def _create_correlation_spectrum_S(self):
        ...
        plot.img_plot(
            ("image"), xbounds=xb, ybounds=yb, colormap=jet, name="image"
        )  # how to set xy bounds???
    ...
    def _create_figureContainer(self):
        ...
        subContainert1 = HPlotContainer(
            self.scatterPlot_t1_start,
            self.scatterPlot_t1_end,
            height=270,
            resizable="h",
            padding=0,
        )
        subContainert2 = HPlotContainer(
            self.scatterPlot_t2_start,
            self.scatterPlot_t2_end,
            height=270,
            resizable="h",
            padding=0,
        )
        ...
        self.figureContainer = VPlotContainer(
            subContainert2, subContainert1, subContainer_top, padding=0
        )
    ...
    def update_correlation_spectrum_S_axis(self):
        ...
        self.correlation_spectrum_S.components[0].index.set_data(
