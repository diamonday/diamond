...
from traits.api import String, Range, Int, Str, Float, Bool, Tuple, Array, Instance, Property, Enum, on_trait_change, Button, DelegatesTo
...
class Polarization_QND(ManagedJob, GetSetItemsMixin):
    ...
    # Readout
    frequency = Range(low=1, high=20e9, value=1.454545e9, desc='microwave frequency', label='frequency mw[Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    ...
    read_count_low = Range(low=0, value=200, high=20000, desc="Min Count", label="Min Count", mode='text', auto_set=False, enter_set=True)
    read_count_high = Range(low=1, value=400, high=20000, desc="Max Count", label="Max Count", mode='text', auto_set=False, enter_set=True)
    ...
    initLaser_length = Range(low=0.0,  high=10000.0, value=300.0, desc='Init Laser Length', label='Init Laser Length [ns]', mode='text', auto_set=False, enter_set=True)
    reInitLaser_length = Range(low=0.0,  high=10000.0, value=300.0, desc='RE-init Laser Length', label='Re-init Laser Length [ns]', mode='text', auto_set=False, enter_set=True)
    laser_length = Range(low=0.0,  high=10000.0, value=300.0, desc='Readout Laser Length', label='Laser Length [ns]', mode='text', auto_set=False, enter_set=True)
    ...
    tau_DD = Range(low=1.5, high=1.0e+6, value=2.0e+3, desc='tau DD',label='tau DD[ns]', mode='text',auto_set=False, enter_set=True)
    tau_f = Range(low=1.5, high=1.0e+6, value=2.0e+3, desc='tau f',label='tau f[ns]', mode='text',auto_set=False, enter_set=True)
    tau_f_ref = Range(low=1.5, high=1.0e+6, value=2.0e+3, desc='tau',label='tau f ref[ns]', mode='text',auto_set=False, enter_set=True)
    N_DD = Range(low=1, high=1000, value=1, desc='number of DD block', label='N DD', mode='text', auto_set=False, enter_set=True)
    repetition = Range(low=0, high=100000, value=10, desc='repetition', label='repetition', mode='text', auto_set=False, enter_set=True)
    
    pulpol = Bool(False, desc="Use PulPol for initialization", label="Use PulPol")
    tau_PP = Range(low=1.5, high=1.0e+6, value=2.0e+3, desc='tau PulPol',label='tau PulPol[ns]', mode='text',auto_set=False, enter_set=True)
    t_rabi = Range(low=0., high=100000., value=200., desc='MW period before PulPol', label='t rabi [ns]', mode='text', auto_set=False, enter_set=True)
    laser_length_PP = Range(low=1., high=100000., value=300., desc='laser length PulPol [ns]', label='laser PulPol[ns]', mode='text', auto_set=False, enter_set=True)
    wait_length_PP = Range(low=0., high=100000., value=300., desc='wait length PulPol [ns]', label='wait PulPol[ns]', mode='text', auto_set=False, enter_set=True)
    repetition_PP = Range(low=0, high=100000, value=10, desc='repetition PulPol', label='repetition PulPol', mode='text', auto_set=False, enter_set=True)
    N_PP = Range(low=1, high=1000, value=1, desc='number of PulPol block', label='N PP', mode='text', auto_set=False, enter_set=True)
    ...
    n_batch = Range(low=0, value=200, high=20000, desc="# runs per batch", label="# runs per batch", mode='text', auto_set=False, enter_set=True)
    ...
    def generate_sequence(self):
        ...
        Init_PP = [(['aom'], laser_length_PP), ([], wait_length_PP), ]
        ...
        if self.pulpol:
            ...
            sequence0 = Init + (Delay_PP + PulPol0 + Init + DDseq + Corrseq_ref + DDseq + Readout + reInit + Delay_ref)*int(repetition)
            if t_rabi == "0":
                # sequence1 = Init + PulPol0 + Init + (reInit + DDseq + Corrseq + DDseq + Readout + Delay)*int(repetition)
                sequence1 = Init + (PulPol0 + Init + DDseq + Corrseq + DDseq + Readout + reInit + Delay)*int(repetition)
            else:
                # sequence1 = Init + PulPol1 + Init + (reInit + DDseq + Corrseq + DDseq + Readout + Delay)*int(repetition)
                sequence1 = Init + (PulPol1 + Init + DDseq + Corrseq + DDseq + Readout + reInit + Delay)*int(repetition)
        else:
            sequence0 = Init + (DDseq + Corrseq_ref + DDseq + Readout + reInit + Delay_ref)*int(repetition)
            sequence1 = Init + (DDseq + Corrseq + DDseq + Readout + reInit + Delay)*int(repetition)
    ...
    def _run(self):
        ...
        try:
            ...
            tagger = self.time_tagger.Pulsed(n_bins, int(np.round(self.laser_length * 1000)), self.n_laser + self.n_laser_ref, tt_ch_apd, tt_ch_laser, tt_ch_seq)
    ...
    def _updateSeq_fired(self):
        ...
        BinSeq = self.pg.convertSequenceToBinary(self.sequence[0] + self.sequence[1],loop=True)
    ...
    get_set_items = [
        'pulpol','tau_PP','t_rabi','laser_length_PP',
        'repetition_PP','N_PP','wait_length_PP',
        'use_stored_sequence','sequence_path',
        'frequency','power_mw','t_2pi_x', 't_2pi_y',
        'tau_DD','tau_f','tau_f_ref','N_DD','repetition',
        'reInitLaser_length','initLaser_length','laser_length','wait_length',
        'read_time','read_runs_round','read_count_low',
        'read_count_high','read_count_delta',
        'save_time_trace', 'time_trace_path','__doc__',
        'runs', 'data_bins', 'count_data', 'sequence',
        'T_seq', 'T_seq_ref','keep_data','n_batch',
        'n_laser','n_laser_ref'
    ]
    ...
    traits_view = View(
        VGroup(
            ...
            HGroup(
                VGroup(
                    VGroup(
                        Item('initLaser_length', width= -80, enabled_when='state != "run"'),
                        Item('reInitLaser_length', width= -80, enabled_when='state != "run"'),
                    ...
                    VGroup(
                        ...
                        Item('laser_length_PP', width= -80, enabled_when='state != "run"'),
                        Item('wait_length', width= -80, enabled_when='state != "run"'),
