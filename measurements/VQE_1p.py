...
class VQE_1p(Pulsed):
    ...
    def __init__(self, pulse_generator, time_tagger, microwave, microwave2, awg_rf, **kwargs):
        ...
        super(VQE_1p, self).__init__(pulse_generator, time_tagger, microwave, microwave2, awg_rf, **kwargs)
        ...
        try:
            if (self.auto_save == True):
                if (self.state == 'run'):
                    ...
                    dir2 = '/vqe_%03.3f_%03.3f_%imin_%i.pys' % (self.rot_n_y, self.rot_n_x, self.saveWaitTime, self.save_count_name)
    ...
    def generate_heating_time(self):
        ...
        max_time = max(hard_pi_t + t_rot + mw_pi + mw_pi_rf , mw_pi_rf_12, hard_pi_t + t_rot + hard_pi_t + mw_pi2_rf + mw_pi_2)
        ...
        "Non-diag terms"
        'rho23 +pi/2'
        heating_time.append(max_time - hard_pi_t - t_rot - hard_pi_t - mw_pi2_rf - mw_pi)
    ...
    def VQE_wfm_rf(self,j):
        ...
        if j == 0:
            ...
            'rho23 +pi/2'
            pulse_sequence_rf = [carry_hard_pi] + pulse_rot + [carry_hard_pi] + pi2y_rf_p + [carry_pi24] + [rf_heating] + [zero_rf]
            name_rf = 'VQE_rho23_p'
            wave_rf = Waveform610(name_rf, pulse_sequence_rf, offset=0, file_type=0, sampling=self.sampling_rf)# 0 for wfm file and 1 for pat file
            ...
            'rho23 -pi/2'
            pulse_sequence_rf = [carry_hard_pi] + pulse_rot + [carry_hard_pi] + pi2y_rf_m + [carry_pi24] + [rf_heating] + [zero_rf]
            name_rf = 'VQE_rho23_m'
            wave_rf = Waveform610(name_rf, pulse_sequence_rf, offset=0, file_type=0, sampling=self.sampling_rf)# 0 for wfm file and 1 for pat file
            ...
            'rho14 +pi/2'
            pulse_sequence_rf = [carry_hard_pi] + pulse_rot + [zero_rf] + pi2y_rf_p + [carry_pi13] + [rf_heating] + [zero_rf]
            name_rf = 'VQE_rho14_p'
            wave_rf = Waveform610(name_rf, pulse_sequence_rf, offset=0, file_type=0, sampling=self.sampling_rf)# 0 for wfm file and 1 for pat file
            ...
            'rho14 -pi/2'
            pulse_sequence_rf = [carry_hard_pi] + pulse_rot + [zero_rf] + pi2y_rf_m + [carry_pi13] + [rf_heating] + [zero_rf]
            name_rf = 'VQE_rho14_m'
            wave_rf = Waveform610(name_rf, pulse_sequence_rf, offset=0, file_type=0, sampling=self.sampling_rf)# 0 for wfm file and 1 for pat file
    ...
    get_set_items = Pulsed.get_set_items + [ 'splitting', 'y1', 'spin_state', 'rot_n_y', 'rot_n_x','rot_e_y', 'rot_e_x', 'main_dir_rf', 'amp1_rf','amp_rf_34','amp_rf_12', 'freq_rf_34', 'freq_rf_12',  't_2pi_rf_34', 't_2pi_rf_12', 't_2pi_mw_13',  't_2pi_mw_24', 'freq_awg_rf_heating', 'saveWaitTime', 'auto_save_dir', 'save_count_name', 'one_round_time']
                ...
                VGroup(
                    ...
                    HGroup(
                        HGroup(
                            ...
                            Item('t_2pi_mw_24', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_func=lambda x:'%.2f'%x), width=-80),
                            label='SMIQ',
                            show_border=True,
                        ),
                    ),
