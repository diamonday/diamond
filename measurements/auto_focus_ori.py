...
import numpy as np
from traits.api import Instance, Range, Bool, Array, Str, Enum, Button, on_trait_change
...
class AutoFocus(ManagedJob, GetSetItemsMixin):
    ...
    def _run(self):
        ...
        try:
            ...
            if self.current_target is None:
                # if no point is specified (None), just focus around the current point
                self.focus()
            else:  # focus target
    ...
    def focus(self):
        ...
        """
        xp = self.confocal.x  # xp means xprevious
        yp = self.confocal.y
        zp = self.confocal.z
        ...
        self.data_xy = np.zeros((len(Y), len(X)))  # Y rows, X columns
        ...
        for i, y in enumerate(Y):
            if threading.current_thread().stop_request.isSet():
                ...
                self.confocal.z = zp
                return xp, yp, zp
            # scan points reversed from the 2nd line (i=0 is the first line)
            # we reverse the line's position vector directly, not in the PosToVolt function in nidaq.py. (It's useless actually.)
            if i % 2 != 0:
                XL = XP
            else:
                XL = X
            YL = y * np.ones(
                X.shape
            )  # for a horizontal line, we now iterate its y coordinate
            ZL = zp * np.ones(X.shape)
            Line = np.vstack((XL, YL, ZL))  # 3 rows matrix   X row, Y row and Z row
        ...
        # after scanning a xy plane, we go to the best xy position and scan a line in the z direction.
        Z = np.hstack(
            (
