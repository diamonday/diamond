...
class VQE_test(Pulsed):
    ...
    def __init__(self, pulse_generator, time_tagger, microwave, awg, awg_rf, **kwargs):
        ...
        super(VQE_test, self).__init__(pulse_generator, time_tagger, microwave, awg, awg_rf, **kwargs)
    ...
    def generate_sequence(self):
        ...
        norm_sequences.append(([], t_pi_combined + t_2pi_rf_34/2 + t_pi_combined)) # N2  

        # polarization rates
        pol_sequences = []
        pol_sequences.append(([], t_2pi_rf_34/2 + t_pi_combined)) # first pi_34 then Big Pi, for e
        pol_sequences.append(([], t_2pi_mw_24/2)) # pi_24 for n
        pol_sequences.append(([], t_2pi_rf_34/2 + t_pi_combined + t_2pi_rf_34/2 + t_2pi_mw_24/2)) # pi_34, Big Pi, pi_34, pi_24, for n
        pol_sequences.append(([], t_2pi_rf_34/2 + t_pi_combined + t_2pi_rf_34/2)) # pi_34, Big Pi, pi_34, for n
    ...
    def VQE_wfm(self,i,j):
        ...
        if j == 0:
            ...
            if int(t_pi_combined_rft) < 512:
                ...
                carry = Idle(0)
                n, carry.duration = int(t_pi_combined_rft) / 512, int(t_pi_combined_rft) % 512
    ...
    def _create_line_plot(self):
        ...
        self.index_range = DataRange1D(self.x_data, low_setting=-0.5, high_setting=11 + self.test_n +  0.5, stretch_data=False)
