...
class ODMR(ManagedJob, GetSetItemsMixin):
    ...
    # Drift
    frequency_origin = Float(value=-1., label='Initial Frequency')
    frequency_drift = Array(value=np.array([0.]))
    ...
    def __init__(self, *args):
        ...
        self.on_trait_change(self._update_drift_data_value, 'frequency_drift', dispatch='ui')
    ...
    def _run(self):
        ...
        try:
            ...
            if self.pulsed:
                ...
                ha.PulseGenerator().Sequence(100 * [ (['laser', 'aom'], self.laser), ([], self.wait), (['mw_all_x'], self.t_pi) ]) #for VQE mw and rf circuit with 3 smiq
            else:
                #ha.PulseGenerator().Continuous(['A','green','mw_x'])
                ha.PulseGenerator().Continuous(['green','mw_all_x']) #for VQE mw and rf circuit with 3 smiq
            ...
            if not self.randomize:
                ...
                ha.MicrowaveC().initSweep(self.frequency, _power * np.ones(n)) # for 3 smiq
            else:
                _rand_run_remains = 0
                ...
                if self.periodic_focus:
                    ...
                    if not np.isnan(fp):
                        if self.frequency_origin <= 0:
                            ...
                            self.frequency_drift = np.append(self.frequency_drift, current_drift)
    ...
    def _create_line_plot(self):
        ...
        line_plot = Plot(line_data, width=100, height=100, padding=8, padding_left=64, padding_right=16, padding_bottom=32)
    ...
    def _create_matrix_plot(self):
        ...
        matrix_plot = Plot(matrix_data, width=100, height=20, padding=8, padding_left=64, padding_right=16, padding_bottom=32)
    ...
    def _drift_plot_default(self):
        ...
        plot = Plot(self.drift_data, width=100, height=20, padding=8, padding_left=64, padding_right=16, padding_bottom=32)
    ...
    def _forget_drift_fired(self):
        ...
        self.frequency_drift = np.array([0.])
    ...
    traits_view = View(
        VGroup(
            ...
            VGroup(
                HGroup(
                    ...
                    Item('periodic_focus'),
                    Item(
                        'frequency_origin', style='readonly', visible_when='frequency_origin > 0',
                ...
                Tabbed(
                    ...
                    VGroup(
                        HGroup(
                            Item('power', width= -40, enabled_when='state != "run" and pulsed == False')
                        ),
                        HGroup(
        ...
        title='ODMR', width=500, height=700, buttons=[],
        x=0, y=1080-740,
        resizable=True, handler=ODMRHandler,
    )
    ...
    get_set_items = [
        ...
        'drift_time','frequency_drift','frequency_origin',
        '__doc__'
    ]
