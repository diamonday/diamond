...
class NuclearRabi_smiq_Nnorm( Pulsed ):
    ...
    """use one smiq and use N1 and N4 count to normlize and measure the N4 count in the middle of the sequence"""
    ...
    get_set_items = Pulsed.get_set_items + [ 'freq_awg_rf', 'amp1_rf', 'show_fit', 'fit_parameters', 'rabi_frequency', 'rabi_period',
        'rabi_offset', 'rabi_contrast', 'pi2', 'pi', 'pi32',  'mw_pi',  'mw_pi_rf'
    ]
    
    def __init__(self, pulse_generator, time_tagger, microwave, awg_rf, **kwargs):
        self.dual = False
        super(NuclearRabi_smiq_Nnorm, self).__init__(pulse_generator, time_tagger, microwave, awg_rf, **kwargs)
        self.show_fit = True
    ...
    def generate_sequence(self):
        ...
        insert_pos  = int((mw_pi_rf - start_time)/time_step) + 1
        ...
        sequence = [
            (['B'], max_time + trig_interval_rf + trig_delay_rf + mw_pi + mw_pi - 100),
            (['B','aom'], laser),
            (['B','aom'], (576) * 1e9 /self.sampling_rf + 450 ),
            (['B'], wait )
        ]
        
        sequence.append((['B'], max_time + trig_interval_rf + trig_delay_rf + mw_pi + mw_pi))
        ...
        for t in tau[:insert_pos]:
            sequence.append((['B'], max_time - t)) #consider (['sequence'], 100)
            sequence.append((['B','awg_rf'],  trig_interval_rf))
            sequence.append((['B'], trig_delay_rf))
            sequence.append((['B','A','mw_x'], mw_pi))
            sequence.append((['B'], t))
            sequence.append((['B','A','mw_x'], mw_pi))
            sequence.append((['B','laser', 'aom'], laser))
            sequence.append((['B','aom' ], (576) * 1e9 /self.sampling_rf + 450 )) #for trigger dead time of awg610(for awg520, we can ignore it if we set the sampling rate as 1e9)
        ...
        #the spinstate[insert_pos] is the N4 count
        sequence.append((['B'], max_time - mw_pi_rf))
        sequence.append((['B','awg_rf'],  trig_interval_rf))
        sequence.append((['B'], trig_delay_rf))
        sequence.append((['B','A','mw_x'], mw_pi))
        sequence.append((['B'], mw_pi_rf))
        sequence.append((['B','A','mw_x'], mw_pi))
        sequence.append((['B','laser', 'aom'], laser))
        sequence.append((['B','aom' ], (576) * 1e9 /self.sampling_rf + 450 )) #for trigger dead time of awg610(for awg520, we can ignore it if we set the sampling rate as 1e9)
        sequence.append((['B'], wait))
    ...
    def compile_waveforms(self):
        ...
        time_step = self.time_step
        mw_pi_rf = self.mw_pi_rf
        insert_pos  = int((mw_pi_rf - start_time)/time_step)
        ...
        for i,t in enumerate(tau[:insert_pos]):
        ...
        for i,t in enumerate(tau[insert_pos:]):
            t = t*self.sampling_rf/1e9
            mw_rf.duration = t
            name_rf = 'NRABI_%03i' % (i + insert_pos)
            #waves = IQWaveform(name, pulse_sequence, file_type=0) # 0 for wfm file and 1 for pat file
            wave_rf = Waveform610(name_rf, pulse_sequence_rf, offset=0, file_type=0, sampling=self.sampling_rf)# 0 for wfm file and 1 for pat file
            self.waves_rf.append(wave_rf)
            seq_rf.append(wave_rf, wait=True)
    ...
    def _analyze_count_data(self):
        ...
        mw_pi_rf = self.mw_pi_rf
        insert_pos  = int((mw_pi_rf - start_time)/time_step) + 1
        y1_1 = y1[1:insert_pos]
        y1_2 = y1[insert_pos+1:]
        self.y1 = np.concatenate((y1_1, y1_2))
    ...
    traits_view = View(
        ...
        title='NuclearRabi_Nnorm (no awg520)',
        buttons=[],
        resizable=True,
        width =1400,
        height=800,
