...
class ODMR(Pulsed):
    ...
    def __init__(self, pulse_generator, time_tagger, microwave, awg, **kwargs):
        ...
        self.frequency = np.arange(
            self.frequency_begin_p, self.frequency_end_p, self.frequency_delta_p
        )
        self.modify_plot()
    ...
    def apply_parameters(self):
        ...
        if (
            ...
            and np.any(frequency == self.frequency)
        ...
        if self.show_fit:
            ...
            try:
                p = fitting.fit_multiple_lorentzians(
                    self.frequency, self.y1, N, threshold=self.threshold * 0.01
                )
            except Exception:
