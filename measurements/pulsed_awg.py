...
class Pulsed( ManagedJob, GetSetItemsMixin ):
    ...
    main_dir = String('\waves', desc='.WFM/.PAT folder in AWG', label='Waveform Dir.:', mode='text')
    ...
    vpp1       = Range(low=0.0, high=2.0, value=2.0, label='vpp CH1 [V]', desc='Set output voltage peak to peak [V] of CH1.', mode='text', auto_set=False, enter_set=True)
    vpp2       = Range(low=0.0, high=2.0, value=2.0, label='vpp CH2 [V]', desc='Set output voltage peak to peak [V] of CH2.', mode='text', auto_set=False, enter_set=True)
    trig_interval = Range(low=11.0, high=10000.0, value=28.0, label='Trig Interval', desc='Set the trigger duration for the AWG.', mode='text', auto_set=False, enter_set=True)
    trig_delay = Range(low=11.0, high=10000.0, value=45.0, label='Trig Delay', desc='Trigger delay of the AWG.', mode='text', auto_set=False, enter_set=True)
    sampling   = Range(low=50.0e3, high=1.0e9, value=1.0e9, label='sampling [Hz]', desc='Set the sampling rate of the AWG.', mode='text', auto_set=False, enter_set=True)
    waves      = []
    ...
    wait  = Range(low=1., high=5e6, value=1000., desc='wait [ns]', label='wait [ns]', mode='text', auto_set=False, enter_set=True)
    
    freq  = Range(low=1, high=20e9,  value=2.70e9, desc='SMIQ frequency', label='SMIQ frequency [Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    ...
    end_time   = Range(low=1., high=2e7, value=500., desc='end time [ns]', label='end time [ns]', mode='text', auto_set=False, enter_set=True)
    time_step  = Range(low=1., high=1e6, value=5., desc='time step [ns]', label='time step [ns]', mode='text', auto_set=False, enter_set=True)
    ...
    record_length                = Range(low=0,  high=10000, value=3000, desc='time window for pulse analysis [ns]',          label='profile length [ns]', mode='text', auto_set=False, enter_set=True)
    time_window_width            = Range(low=10, high=1000,  value=300,  desc='time window for pulse analysis [ns]',          label='signal window [ns]',  mode='text', auto_set=False, enter_set=True)
    time_window_offset_signal    = Range(low=0,  high=1000,  value=25,   desc='offset of time window for signal [ns]',        label='signal at [ns]',      mode='text', auto_set=False, enter_set=True)
    time_window_offset_normalize = Range(low=0,  high=10000, value=2000, desc='offset of time window for normalization [ns]', label='normalize at [ns]',   mode='text', auto_set=False, enter_set=True)
    ...
    # Plots
    line_plot_data   = Instance( ArrayPlotData, transient=True ) # transient: do not save this trait
    ...
    # get_set_items
    get_set_items = [
        'laser', 'wait', 'freq', 'power', 'run_time', # later also sweeps
        'pulse_profile','flank','time_window_width','time_window_offset_signal','time_window_offset_normalize',
        'tau', 'y1', 'y2','count_data',
        'nu', 'yf1', 'yf2',
        'vpp1', 'vpp2', 'sampling','trig_interval', 'trig_delay',
    ...
    def update_fft(self):
        ...
        if n % 2 == 0:
            self.yf1 = transform1[:n/2] + transform1[:n/2-1:-1] # sum both directions
        else:
    ...
    def _create_fft_plot(self):
        fft_plot_data = ArrayPlotData(index=np.array((0,1)), yf1=np.array((0,0)), yf2=np.array((0,0)))
        plot = Plot(fft_plot_data, padding=8, padding_left=64, padding_bottom=36)
        plot.plot(('index','yf1'), color=scheme['data 1'], line_width=2, id='0', name='yf1')
        if self.dual:
            plot.plot(('index','yf2'), color=scheme['data 2'], line_width=2, id='0', name='yf2')
    ...
    def _update_data_depth(self):
        ...
        if self.dual:
            ...
            plot.plot(('index', 'y2'), style='line', line_width=2, color=scheme['data 2'], id='32', name='y2')
        else:
            for i, c in enumerate(plot.components):
                if c.id == '32': plot.remove(plot.components[i])
        plot.request_redraw()
...
class Rabi( Pulsed ):
    ...
    get_set_items = Pulsed.get_set_items + [
        'freq_tot', 'freq_awg', 'offset', 'amp1', 'show_fit', 'fit_parameters', 'rabi_frequency', 'rabi_period',
        'rabi_offset', 'rabi_contrast', 'pi2', 'pi', 'pi32', 'phase_sin'
    ]
    
    def __init__(self, pulse_generator, time_tagger, microwave, awg, **kwargs):
        self.dual = False
        super(Rabi, self).__init__(pulse_generator, time_tagger, microwave, awg, **kwargs)
        self.show_fit = True
...
class Count_decay( Pulsed ):
    ...
    freq_tot = Range(low=1, high=20e9, value=2.87e9, desc='Total frequency', label='Total frequency [Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    phase_sin = Range(low=-100.0, high=100.0, value=0.0, desc='Multiple of pi', label='Phase', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    t_pi = Range(low=1., high=2e7, value=500., desc='pi time [ns]', label='pi time [ns]', mode='text', auto_set=False, enter_set=True)
    ...
    get_set_items = Pulsed.get_set_items + [
        'freq_tot', 'freq_awg', 'offset', 'amp1', 'show_fit', 'fit_parameters', 'rabi_frequency', 'rabi_period',
        'rabi_offset', 'rabi_contrast', 'pi2', 'pi', 'pi32', 'phase_sin', 't_pi'
    ]
    
    def __init__(self, pulse_generator, time_tagger, microwave, awg, **kwargs):
        self.dual = False
        super(Count_decay, self).__init__(pulse_generator, time_tagger, microwave, awg, **kwargs)
        self.show_fit = True
        self.freq_tot = self.freq + self.freq_awg + self.offset
    ...
    def generate_sequence(self):
        ...
        for t in tau:
            ...
            sequence.append((['awg'], trig_interval))
            sequence.append(([ ], trig_delay + t_pi + t))
            # Add switch at the end
            # sequence.append((['mw_y'], trig_delay + t))
            sequence.append((['aom', 'laser'], laser))
            sequence.append(([ ], wait))
    ...
    def compile_waveforms(self):
        ...
        # pulse objects
        mw = Sin(t_pi, freq=f1, amp=self.amp1, phase=phase)
        zero = Idle(1)
...
class Trigger( Pulsed ):
    ...
    head = Range(low=1., high=1e7, value=15, desc='Length of the head [ns]', label='Head [ns]', mode='text', auto_set=False, enter_set=True)
    amp_body = Range(low=0.0, high=1.0, value=1.0, desc='Scale of the body', label='Body amp', auto_set=False, enter_set=True)
    amp1  = Range(low=0.0, high=1.0, value=1.0, desc='Waveform amplitude factor', label='wfm amp', auto_set=False, enter_set=True)
    n_rep = Range(low=0.0, high=10000, value=40, desc='Repetition', label='Repetition', auto_set=False, enter_set=True)
    ...
    get_set_items = Pulsed.get_set_items + [
        'freq_tot', 'freq_awg', 'amp1', 'amp_body', 'head', 'SMIQ', 'MW_X', 'phase_sin',
        'show_fit','fit_parameters', 'rabi_frequency', 'rabi_period', 'IQ', 'n_rep',
    ]
    
    def __init__(self, pulse_generator, time_tagger, microwave, awg, **kwargs):
        self.dual = False
        super(Trigger, self).__init__(pulse_generator, time_tagger, microwave, awg, **kwargs)
        self.show_fit = True
    ...
    def generate_sequence(self):
        ...
        sequence = [(['green'], wait)]
        if self.MW_X:
            sequence.append((['green','blue',], trig_interval))
            sequence.append((['green','mw_x',], t + trig_interval))
        else:
            sequence.append((['green','awg','blue',], trig_interval))
            sequence.append((['green',], t + trig_interval))
            ...
            Tabbed(
                VGroup(
                    ...
                    HGroup(
                        ...
                        Item('freq_awg', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_func=lambda x:'%e'%x), width=-100),
                        Item('phase_sin', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_func=lambda x:'%.2f'%x), width=-40),
                        Item('amp1', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_func=lambda x:'%.2f'%x), width=-40),
                        Item('amp_body', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_func=lambda x:'%.2f'%x), width=-40),
                        label='Waveform',
                        show_border=True,
                    ),
                    HGroup(
                    ...
                    HGroup(
                            Item('vpp1', width=-50),
                            Item('vpp2', width=-50),
                            Item('sampling', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_func=lambda x:'%e'%x), width=-80),
                            Item('trig_interval', width=-40),
                            Item('n_rep', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_func=lambda x:'%i'%x), width=-40),
                            label='AWG',
                            show_border=True,
                        ),
                ),
...
def gen_blocks(seq,tDD,p,name,idle):

    # Minimum free-evolution duration
    t_min = MIN_POINTS['AWG520']
    waves = []
    ...
    n, remainder = tDD // t_min, np.fmod(tDD,t_min)
    # tDD is tau - t_pi/2 !!
    # if tDD is too short, we don't need to divide the sequence
    if n <= 1:
        sub = False
        free = Idle(tDD)
        pulse = [zero, p[s_head]]
        ...
        wfm = IQWaveform(name, pulse, file_type=0)
        waves.append(wfm['I'])
        waves.append(wfm['Q'])
        return waves, sub
        ...
        sub = True
        free = Idle(t_min)
        carry = Idle(remainder)
        wfm_idle = idle

        sub_name = 'SUB_' + name
        sub_seq = Sequence(sub_name)

        p_head = [zero, p[s_head], free]
        t0 = 1
        wfm_head = IQWaveform(sub_name + '_H', p_head, -t0, file_type=0)
        stub_head = wfm_head['I'].stub
        if stub_head > 0:
            t0 += stub_head
            p_head = [zero, Idle(stub_head), p[s_head], free]
            wfm_head = IQWaveform(sub_name + '_H', p_head, -t0, file_type=0)
        sub_seq.append(wfm_head)
        waves.append(wfm_head['I'])
        waves.append(wfm_head['Q'])
        t_head = wfm_head['I'].duration - t0

        t_offset = t_head
        for j,s in enumerate(seq[1:-1]):
            sub_seq.append(wfm_idle, wfm_idle, repeat=(n - 1))
            t_offset += (n - 1)*t_min

            pulse = [carry, p[s], free, carry]
            name_pi = sub_name + '_PI%03i' % (j + 1)
            wfm_pi = IQWaveform(name_pi, pulse, t_offset, file_type=0)
            sub_seq.append(wfm_pi)
            waves.append(wfm_pi['I'])
            waves.append(wfm_pi['Q'])
            t_offset += wfm_pi['I'].duration
            
            sub_seq.append(wfm_idle, wfm_idle, repeat=(n - 1))
            t_offset += (n - 1)*t_min

        p_end = [p[s_end], zero]
        wfm_end = IQWaveform(sub_name + '_E', p_end, t_offset, file_type=0)
        sub_seq.append(wfm_end)
        waves.append(wfm_end['I'])
        waves.append(wfm_end['Q'])

        return (sub_seq, waves), sub
...
class CPMG( Pulsed ):
    ...
    t_2pi = Range(low=0.0, high=2000, value=200., desc='duration of 2pi pulse', label='2pi duration', auto_set=False, enter_set=True)
    n_pi = Range(low=1, high=100, value=4, desc='number of pi pulses', label='n pi', mode='text', auto_set=False, enter_set=True)
    Range(low=0, high=100, value=10, desc='Number of Fourier components', label='K', mode='text', auto_set=False, enter_set=True)

    get_set_items = Pulsed.get_set_items + [
        'freq_tot', 'freq_awg', 'offset', 'amp1', 'show_fit',
        't_2pi','n_pi','phase_sin',
    ]
    
    def __init__(self, pulse_generator, time_tagger, microwave, awg, **kwargs):
        self.dual = False
        super(CPMG, self).__init__(pulse_generator, time_tagger, microwave, awg, **kwargs)
        self.show_fit = True
        self.freq_tot = self.freq + self.freq_awg + self.offset
    ...
    def get_tau(self,t,t_2pi,n_pi,state='dark'):
        ...
        if state == 'dark':
            if tDD < 0:
                return 0.0 ,t_pi2 + (t_pi)*(n_pi) + t_pi2
            else:
                # Chose the convention from Tim's paper
                return tDD ,t_pi2 + (tDD + t_pi + tDD)*(n_pi) + t_pi2
        elif state == 'bright':
            if tDD < 0:
            ...
            Tabbed(
                VGroup(
                    ...
                    VGroup(
                        ...
                        HGroup(
                            Item('t_2pi', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_func=lambda x:'%.2f'%x), width=-80),
...
class CPMG_block( Pulsed ):
    ...
    def __init__(self, pulse_generator, time_tagger, microwave, awg, **kwargs):
        self.dual = False
        super(CPMG_block, self).__init__(pulse_generator, time_tagger, microwave, awg, **kwargs)
        self.show_fit = True
        self.freq_tot = self.freq + self.freq_awg + self.offset
    ...
    def compile_waveforms(self):
        ...
        # pulse objects
        p = {}
        p['pi x'] = Cos(t_pi, freq=f_res, amp=self.amp1, phase=phase)
        p['pi2 y'] = Sin(t_pi2, freq=f_res, amp=self.amp1, phase=phase)
        p['pi y'] = Sin(t_pi, freq=f_res, amp=self.amp1, phase=phase)
        p['3pi2 y'] = Sin(t_3pi2, freq=f_res, amp=self.amp1, phase=phase)

        # pulse sequence
        seq_dark = ['pi2 y'] + ['pi x']*n_pi + ['pi2 y']

        seq_bright = ['pi2 y'] + ['pi x']*n_pi + ['3pi2 y']
        
        Seq_main=Sequence('CPMG')
        self.waves = []

        tmin = MIN_POINTS['AWG520']
        idle = Waveform('IDLE', Idle(tmin), file_type=0)
        self.waves.append(idle)
        for i,t in enumerate(self.tau):
            # increment microwave duration
            tDD_, t_awg = self.get_tau(t,t_2pi,n_pi,state='dark')
            name = 'D%03i' % i

            tDD = self.sampling*tDD_/1e9
            package, sub = gen_blocks(seq_dark,tDD,p,name,idle)
            if sub:
                Seq_sub, waves = package
                Seq_main.append(Seq_sub, wait=True)
                self.waves += waves
                self.waves.append(Seq_sub)
            else:
                wave = package
                Seq_main.append(wave[0],wave[1], wait=True)
                self.waves += wave
        

        for i,t in enumerate(self.tau):
            tDD_, t_awg = self.get_tau(t,t_2pi,n_pi,state='bright')
            name = 'B%03i' % i

            tDD = self.sampling*tDD_/1e9
            package, sub = gen_blocks(seq_bright,tDD,p,name,idle)
            if sub:
                Seq_sub, waves = package
                Seq_main.append(Seq_sub, wait=True)
                self.waves += waves
        ...
        zero = Idle(1)
        pulse_sequencer1 = [zero, p['pi y'], zero]
        namer1 = 'REF1'
        waver1 = IQWaveform(namer1, pulse_sequencer1, file_type=0)
        self.waves.append(waver1['I'])
        self.waves.append(waver1['Q'])
        Seq_main.append(waver1, wait=True)

        self.waves.append(Seq_main)
        self.main_wave = Seq_main.name
...
class FID( Pulsed ):
    ...
    freq1 = Range(low=1, high=20e9, value=2.80e9, desc='microwave frequency transition 1', label='frequency [Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    freq2 = Range(low=1, high=20e9, value=2.95e9, desc='microwave frequency transition 2', label='frequency [Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    amp1  = Range(low=0.0, high=1.0, value=1.0, desc='Waveform amplitude factor transition 1', label='wfm amp', auto_set=False, enter_set=True)
    amp2  = Range(low=0.0, high=1.0, value=1.0, desc='Waveform amplitude factor transition 2', label='wfm amp', auto_set=False, enter_set=True)
    
    pi2_1   = Range(low=1., high=100000., value=30, desc='length of  pi/2 pulse transition 1 [ns]', label='pi/2 [ns]', mode='text', auto_set=False, enter_set=True)
    pi32_1  = Range(low=1., high=100000., value=90, desc='length of 3pi/2 pulse transition 1 [ns]', label='3pi/2 [ns]', mode='text', auto_set=False, enter_set=True)
    
    pi_2  = Range(low=1., high=100000., value=60, desc='length of pi pulse transition 2[ns]', label='pi [ns]', mode='text', auto_set=False, enter_set=True)
    
    dq = Bool(True, label='DQ', desc='create double quantum coherence')
    
    get_set_items = Pulsed.get_set_items + ['freq1', 'freq2', 'amp1', 'pi2_1', 'pi32_1', 'pi_2', 'dq']
    ...
    def compile_waveforms(self):
        ...
        # pulse sequences
        if self.dq:
            sup_seq = [ zero,  p['pi/2 +'], p['pi -'],          ]
            map_seq = [ carry, p['pi -'],   p['pi/2 +'],  zero, ]
            m32_seq = [ carry, p['pi -'],   p['3pi/2 +'], zero, ]
        else:
            sup_seq = [ zero,  p['pi/2 +'],        ]
            map_seq = [ carry, p['pi/2 +'],  zero, ]
            m32_seq = [ carry, p['3pi/2 +'], zero, ]
    ...
    traits_view = View( VGroup(HGroup(Item('submit_button',   show_label=False),
                               ...
                               Tabbed( VGroup( HGroup( HGroup( Item('freq', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_func=lambda x:'%e'%x), width=-80),
                                                               Item('power', width=-40),
                                                               label='SMIQ',
                                                               show_border=True,
                                                             ),
                                                       HGroup( Item('reload_awg'),
                                                               Item('wfm_button', show_label=False),
                                                               Item('upload_progress', style='readonly', format_str='%i'),
                                                               label='AWG',
                                                               show_border=True,
                                               ...
                                               HGroup( Item('freq1', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_func=lambda x:'%e'%x), width=-80),
                                                       Item('amp1', width=-40),
                                                       Item('pi2_1', width=-40),
                                               ...
                                               HGroup( Item('freq2', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_func=lambda x:'%e'%x), width=-80),
                                                       Item('amp2', width=-40),
                                                       Item('pi_2', width=-40),
                                                       Item('dq'),
                                               ...
                                               VSplit(Item('line_plot', editor=ComponentEditor(), show_label=False, width=500, height=300, resizable=True),
                                                      Item('fft_plot', editor=ComponentEditor(), show_label=False, width=500, height=300, resizable=True),
                                                      Item('matrix_plot', editor=ComponentEditor(), show_label=False, width=500, height=300, resizable=True),
                                                     ),
                                               label = 'data',
                                             ),
                                       VGroup( HGroup( HGroup(Item('laser', width=-50),
                                                              Item('wait', width=-50),
                                                              label='Sequence',
                                                              show_border=True,
                                                             ),
                                                       HGroup(Item('vpp1', width=-50),
                                                              Item('vpp2', width=-50),
                                                              Item('sampling', width=-80),
                                                              label='AWG',
                                                              show_border=True,
...
class Hahn( Pulsed ):
    ...
    freq1 = Range(low=1, high=20e9, value=2.80e9, desc='microwave frequency transition 1', label='1: frequency [Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    freq2 = Range(low=1, high=20e9, value=2.95e9, desc='microwave frequency transition 2', label='2: frequency [Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    amp1  = Range(low=0.0, high=1.0, value=1.0, desc='Waveform amplitude factor transition 1', label='1: wfm amp')
    amp2  = Range(low=0.0, high=1.0, value=1.0, desc='Waveform amplitude factor transition 2', label='2: wfm amp')
    
    pi2_1   = Range(low=1., high=100000., value=30, desc='length of pi/2 pulse transition 1 [ns]', label='1: pi/2 [ns]', mode='text', auto_set=False, enter_set=True)
    pi_1    = Range(low=1., high=100000., value=60, desc='length of pi pulse transition 1 [ns]', label='1: pi [ns]', mode='text', auto_set=False, enter_set=True)
    pi32_1  = Range(low=1., high=100000., value=90, desc='length of 3pi/2 pulse transition 1 [ns]', label='1: 3pi/2 [ns]', mode='text', auto_set=False, enter_set=True)
    
    pi_2  = Range(low=1., high=100000., value=60, desc='length of pi pulse transition 2[ns]', label='2: pi [ns]', mode='text', auto_set=False, enter_set=True)
    
    dq = Bool(True, desc='create double quantum coherence')
    
    get_set_items = Pulsed.get_set_items + ['freq1', 'freq2', 'amp1', 'pi2_1', 'pi_1', 'pi32_1', 'pi_2', 'dq']
    ...
    def compile_waveforms(self):
        ...
        for i, t in enumerate(tau):
            if t < stub_a:
                ...
                if self.dual:
                    s32_wfm = IQWaveform('HAHN_SUB_%03i' % (i + len(tau)), sup_seq + ref_seq + m32_seq)
                    waves.append(sub_wfm['I'])
                    waves.append(sub_wfm['Q'])
...
class DEER( Pulsed ):
    ...
    freq1  = Range(low=1,   high=20e9,    value=2.80e9, desc='microwave frequency [Hz]',   label='frequency [Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    ...
    pi2_1  = Range(low=1.,  high=100000., value=30,     desc='length of pi/2 pulse[ns]',   label='pi/2 [ns]',      mode='text', auto_set=False, enter_set=True)
    pi_1   = Range(low=1.,  high=100000., value=60,     desc='length of pi pulse [ns]',    label='pi [ns]',        mode='text', auto_set=False, enter_set=True)
    pi32_1 = Range(low=1.,  high=100000., value=90,     desc='length of 3pi/2 pulse [ns]', label='3pi/2 [ns]',     mode='text', auto_set=False, enter_set=True)
    
    freq2  = Range(low=1,   high=20e9,    value=2.95e9, desc='microwave frequency [Hz]',   label='frequency [Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    amp2   = Range(low=0.0, high=1.0,     value=1.0,    desc='Waveform amplitude factor',  label='wfm amp',        mode='text', auto_set=False, enter_set=True)
    pi_2   = Range(low=1.,  high=100000., value=60,     desc='length of pi pulse[ns]',     label='pi [ns]',        mode='text', auto_set=False, enter_set=True)
    
    freq3  = Range(low=1,   high=20e9,    value=2.95e9, desc='microwave frequency [Hz]',   label='frequency [Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    amp3   = Range(low=0.0, high=1.0,     value=1.0,    desc='Waveform amplitude factor',  label='wfm amp',        mode='text', auto_set=False, enter_set=True)
    pi_3   = Range(low=1.,  high=100000., value=60,     desc='length of pi pulse [ns]',    label='pi [ns]',        mode='text', auto_set=False, enter_set=True)
    
    freq4  = Range(low=1,   high=20e9,    value=2.95e9, desc='microwave frequency [Hz]',   label='frequency [Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    amp4   = Range(low=0.0, high=1.0,     value=1.0,    desc='Waveform amplitude factor',  label='wfm amp',        mode='text', auto_set=False, enter_set=True)
    pi_4   = Range(low=1.,  high=100000., value=60,     desc='length of pi pulse [ns]',    label='pi [ns]',        mode='text', auto_set=False, enter_set=True)
    ...
    echo_time = Range(low=500, high=1e7, value=50000, desc='Free evolution time of sensor spin [ns]', label='echo [ns]', mode='text', auto_set=False, enter_set=True)
    
    get_set_items = Pulsed.get_set_items + ['freq1', 'freq2', 'freq3', 'freq4', 'amp1', 'amp2', 'amp3', 'amp4',
                                            'pi2_1', 'pi_1', 'pi32_1', 'pi_2', 'pi_3', 'pi_4', 'dqa', 'dqb',
                                            'echo_time',
                                           ]
    ...
    def compile_waveforms(self):
        ...
        # sensor
        if self.dqa:
            sup_seq = [        p['pi/2 A+'], p['pi A-'], ]
            ref_seq = [ carry, p['pi A-'],   p['pi A+'],    p['pi A-'], ]
            map_seq = [ carry, p['pi A-'],   p['pi/2 A+'],  zero, ]
            m32_seq = [ carry, p['pi A-'],   p['3pi/2 A+'], zero, ]
        else:
            sup_seq = [        p['pi/2 A+'], ]
            ref_seq = [ carry, p['pi A+'], ]
            map_seq = [ carry, p['pi/2 A+'],  zero, ]
            m32_seq = [ carry, p['3pi/2 A+'], zero, ]
        # emitter
        if self.dqb:
            pre_seq = [ zero,  p['pi B-'], ]
            flp_seq = [ carry, p['pi B-'], p['pi B+'], ]
        else:
            pre_seq = [ zero, ]
            flp_seq = [ carry, p['pi B+'], ]
    ...
    traits_view = View( VGroup(HGroup(Item('submit_button', show_label=False),
                               ...
                               Tabbed( VGroup( VSplit( Item('line_plot', editor=ComponentEditor(), show_label=False, width=500, height=300, resizable=True),
                                                       Item('fft_plot', editor=ComponentEditor(), show_label=False, width=500, height=300, resizable=True),
                                                       Item('matrix_plot', editor=ComponentEditor(), show_label=False, width=500, height=300, resizable=True),
                                                     ),
                                               label = 'data',
                                             ),
                                       VGroup( HGroup( HGroup( Item('freq', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_func=lambda x:'%e'%x), width=-80),
                                                               Item('power', width=40),
                                                               label = 'SMIQ',
                                                               show_border = True,
                                                             ),
                                               ...
                                               VGroup( HGroup( Item('freq1', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_func=lambda x:'%e'%x), width=-80),
                                                               Item('pi2_1', width=40),
                                                               Item('pi_1', width=40),
                                                               Item('pi32_1', width=40),
                                                               Item('amp1', width=40),
                                               ...
                                               VGroup( HGroup( Item('freq3', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_func=lambda x:'%e'%x), width=-80),
                                                               Item('pi_3', width=40),
                                                               Item('amp3', width=40),
                                                               label = 'Transition 1',
                                                               show_border = True,
                                                             ),
                                                       HGroup( Item('freq4', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_func=lambda x:'%e'%x), width=-80),
                                                               Item('pi_4', width=40),
                                                               Item('amp4', width=40),
                                                               Item('dqb'),
                                                               label = 'Transition 2',
                                               ...
                                               HGroup( Item('echo_time', width=40),
                                                       Item('start_time', width=40),
                                                       Item('end_time', width=40),
                                                       Item('time_step', width=40),
                                                       label = 'Sweep',
