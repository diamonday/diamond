...
class Rho23Rabi( Pulsed ):
    ...
    def generate_sequence(self):
        ...
        sequence.append(([], max_time + 2 * mw_pi + mw_pi2_rf)) # the typical time between the laser illumination is : wait + max_time + trig_interval_rf + trig_delay_rf + hard_pi_t + mw_pi + mw_pi_rf
    ...
    def compile_waveforms(self):
        ...
        for i,t in enumerate(self.tau):
            ...
            rf_heating.duration = int(int(heating_time/T_heating) * T_heating)
            pulse_sequence_rf = [carry_pi13] + [mw_rf] + pi2y_rf_p + [carry_pi13] + [rf_heating] + [zero_rf]
            ...
            heating_time = max_time - t
            rf_heating.duration = int(int(heating_time/T_heating) * T_heating)
            pulse_sequence_rf = [carry_pi13] + [mw_rf] + pi2y_rf_m + [carry_pi13] + [rf_heating] + [zero_rf]
