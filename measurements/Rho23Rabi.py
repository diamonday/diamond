...
class Rho23Rabi(Pulsed):
    ...
    def generate_sequence(self):
        ...
        sequence = [
            ...
            (
                [],
                max_time + 2 * mw_pi + mw_pi2_rf - 100,
        ...
        sequence.append(([], max_time - mw_pi_rf + mw_pi + mw_pi2_rf))
        sequence.append((["laser", "aom"], laser))
        sequence.append(
            (["aom"], (576 + 512) * 1e9 / self.sampling_rf + 450)
        )  # for trigger dead time of awg610(for awg520, we can ignore it if we set the sampling rate as 1e9)
    ...
    def compile_waveforms(self):
        ...
        # pulse objects
        mw_rf = Sin(0, freq=f1_rf, amp=self.amp1_rf, phase=np.pi / 2.0)
        zero_rf = Idle(1)
        ...
        mw_rf_pi34 = Sin(mw_pi_rf_rft, freq=f1_rf, amp=self.amp1_rf, phase=np.pi / 2.0)
        ...
        pi2y_rf_p = [
            Sin(duration_pi2y_rf, freq=f1_rf, amp=self.amp1_rf, phase=np.pi / 2.0)
        ]
        pi2y_rf_m = [
            Sin(duration_pi2y_rf, freq=f1_rf, amp=self.amp1_rf, phase=np.pi * 3 / 2.0)
        ]
