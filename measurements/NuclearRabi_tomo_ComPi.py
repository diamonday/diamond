...
class NuclearRabi_smiq_Nnorm( Pulsed ):
    ...
    """2smiq and use the VQE tomo method to read nuclear rabi"""
    ...
    get_set_items = Pulsed.get_set_items + [ 'freq_awg_rf', 'amp1_rf', 'show_fit', 'fit_parameters', 'rabi_frequency', 'rabi_period',
        'rabi_offset', 'rabi_contrast', 'pi2', 'pi', 'pi32',  'mw_pi', 'mw_pi_2',  'mw_pi_rf', 'splitting'
    ]
    ...
    def generate_sequence(self):
        ...
        sequence = [
            ([], max_time + trig_interval_rf + trig_delay_rf + hard_pi_t + mw_pi + mw_pi_rf - 100),#consider (['sequence'], 100)#using AOM so no need to consider duty cycle
            (['aom'], laser),
            (['aom'], (576+120000) * 1e9 /self.sampling_rf + 450 ),#120000 is for microwave heating
            ([], wait )
        ]
        ...
        'N1'
        #using AOM so no need to consider duty cycle
        sequence.append(([], max_time + trig_interval_rf + trig_delay_rf + hard_pi_t + mw_pi + mw_pi_rf)) # the typical time between the laser illumination is : wait + max_time + trig_interval_rf + trig_delay_rf + hard_pi_t + mw_pi + mw_pi_rf
        ...
        'N3'
        #using AOM so no need to consider duty cycle
        sequence.append(([], max_time + trig_interval_rf + trig_delay_rf + mw_pi + mw_pi_rf))
        sequence += hard_pi_seq
        sequence.append((['laser', 'aom'], laser))
        sequence.append((['aom' ], (576+120000) * 1e9 /self.sampling_rf + 450 )) #for trigger dead time of awg610(for awg520, we can ignore it if we set the sampling rate as 1e9)
        sequence.append(([], wait))
        ...
        'N4'
        #using AOM so no need to consider duty cycle
        sequence.append(([], max_time + mw_pi))
        sequence.append((['awg_rf'],  trig_interval_rf))
        sequence.append(([], trig_delay_rf))
        sequence += hard_pi_seq
        sequence.append(([], mw_pi_rf))
        ...
        for t in tau:
            ...
            'Tomo Pi'
            #using AOM so no need to consider duty cycle
            sequence.append(([], max_time - t - mw_pi_2 + mw_pi + mw_pi_rf)) 
            sequence.append((['awg_rf'],  trig_interval_rf))
            sequence.append(([], trig_delay_rf))
            sequence += hard_pi_seq
            sequence.append(([], t))
    ...
    def compile_waveforms(self):
        ...
        mw_pi_2_rft = self.mw_pi * self.sampling_rf / 1e9
    ...
    traits_view = View(
        ...
        title='NuclearRabi 2 smiq and awg610 tomo method',
        buttons=[],
        resizable=True,
        width =1400,
        height=800,
