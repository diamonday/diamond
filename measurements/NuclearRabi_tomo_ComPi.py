    ...
    def generate_sequence(self):
        ...
        sequence = [
            (
                ...
                + mw_pi_rf
                - 100,
            ),  # consider (['sequence'], 100)#using AOM so no need to consider duty cycle
            (["aom"], laser),
            (
                ["aom"],
                (576 + 120000) * 1e9 / self.sampling_rf + 450,
            ),  # 120000 is for microwave heating
            ([], wait),
        ]
        ...
        )  # the typical time between the laser illumination is : wait + max_time + trig_interval_rf + trig_delay_rf + hard_pi_t + mw_pi + mw_pi_rf
        ...
        # using AOM so no need to consider duty cycle
        sequence.append(
            ([], max_time + trig_interval_rf + trig_delay_rf + mw_pi + mw_pi_rf)
        )
        sequence += hard_pi_seq
        sequence.append((["laser", "aom"], laser))
        sequence.append(
