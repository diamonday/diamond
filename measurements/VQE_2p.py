...
class VQE_2p(Pulsed):
    ...
    def generate_heating_time(self):
        ...
        max_time = max(hard_pi_t + t_rot_rf + mw_wait + t_rot_e_y + t_rot_e_x + mw_wait + mw_pi + mw_pi_rf , mw_pi_rf_12, hard_pi_t + t_rot_rf + mw_wait + t_rot_e_y + t_rot_e_x + hard_pi_2_t + mw_wait + mw_pi2_rf + mw_wait + mw_pi_2 + mw_wait)
        ...
        "Non-diag terms"
        'rho23 +pi/2'
        heating_time.append(max_time - hard_pi_t - t_rot_rf - mw_wait - t_rot_e_y - t_rot_e_x - hard_pi_2_t - mw_wait - mw_pi2_rf - mw_wait - mw_pi - mw_wait)
    ...
    def VQE_wfm_rf(self,j):
        ...
        pi2y_rf_p = [Sin(duration_pi2y_rf, freq=freq_rf_34, amp=self.amp_rf_34, phase= np.pi/2.)]
        pi2y_rf_m = [Sin(duration_pi2y_rf, freq=freq_rf_34, amp=self.amp_rf_34, phase= np.pi*3/2.)]
        ...
        t_makeup1 = hard_pi_t_rft + self.rot_n_y * t_2pi_rf_34 + self.rot_n_x * t_2pi_rf_34 - carry_hard_pi.duration - rot_ny_duration - 1 - rot_nx_duration # 1 is for zero_rf in pulse_rot
        carry_makeup1.duration = int(t_makeup1) 

        carry_makeup2 = Idle(0)
        t_makeup2 = 2 * (mw_wait_rf - carry_wait.duration) + t_mw_rot - carry_mw_rot.duration + hard_pi_2_t_rft - carry_hard_pi_2.duration + t_2pi_rf_34/4 - duration_pi2y_rf
        ...
        carry_hard_pi_period1.duration = int((int(carry_hard_pi.duration/T_rf_34) + 1.25 ) * T_rf_34)
        ...
        t_before_pi2_rf = carry_hard_pi_period1.duration + rot_ny.duration + carry_makeup_period2.duration + rot_nx.duration + carry_makeup1.duration + carry_wait.duration + carry_mw_rot.duration + carry_wait.duration + carry_hard_pi_2.duration
        carry_makeup_period3.duration = int(1.25 * T_rf_34 - (t_before_pi2_rf%T_rf_34))
        




        pulse_rot = [rot_ny, carry_makeup_period2, rot_nx, carry_makeup1, carry_wait, carry_mw_rot, carry_wait]
        ...
        if j == 0:
            ...
            'rho23 +pi/2'
            pulse_sequence_rf = [carry_hard_pi_period1] + pulse_rot + [carry_hard_pi_2] + [carry_makeup_period3]  + pi2y_rf_p + [carry_makeup2, carry_wait, carry_pi13, carry_wait] + [rf_heating] + [zero_rf]
            name_rf = 'VQE_rho23_p'
            wave_rf = Waveform610(name_rf, pulse_sequence_rf, offset=0, file_type=0, sampling=self.sampling_rf)# 0 for wfm file and 1 for pat file
            ...
            'rho23 -pi/2'
            pulse_sequence_rf = [carry_hard_pi_period1] + pulse_rot + [carry_hard_pi_2] + [carry_makeup_period3] + pi2y_rf_m + [carry_makeup2, carry_wait, carry_pi13, carry_wait] + [rf_heating] + [zero_rf]
            name_rf = 'VQE_rho23_m'
            wave_rf = Waveform610(name_rf, pulse_sequence_rf, offset=0, file_type=0, sampling=self.sampling_rf)# 0 for wfm file and 1 for pat file
            ...
            'rho14 +pi/2'
            pulse_sequence_rf = [carry_hard_pi] + pulse_rot + [zero_rf] + pi2y_rf_p + [carry_makeup3, carry_wait, carry_pi13, carry_wait] + [rf_heating] + [zero_rf]
            name_rf = 'VQE_rho14_p'
            wave_rf = Waveform610(name_rf, pulse_sequence_rf, offset=0, file_type=0, sampling=self.sampling_rf)# 0 for wfm file and 1 for pat file
            ...
            'rho14 -pi/2'
            pulse_sequence_rf = [carry_hard_pi] + pulse_rot + [zero_rf] + pi2y_rf_m + [carry_makeup3, carry_wait, carry_pi13, carry_wait] + [rf_heating] + [zero_rf]
            name_rf = 'VQE_rho14_m'
            wave_rf = Waveform610(name_rf, pulse_sequence_rf, offset=0, file_type=0, sampling=self.sampling_rf)# 0 for wfm file and 1 for pat file
    ...
    get_set_items = Pulsed.get_set_items + [ 'splitting', 'y1', 'spin_state', 'rot_n_y', 'rot_n_x','rot_e_y', 'rot_e_x', 'main_dir_rf', 'amp1_rf','amp_rf_34','amp_rf_12', 'freq_rf_34', 'freq_rf_12',  't_2pi_rf_34', 't_2pi_rf_12', 't_2pi_mw_13',  't_2pi_mw_24', 'freq_awg_rf_heating', 'saveWaitTime', 'auto_save_dir', 'save_count_name', 'one_round_time', 't_2pi_mw_all_y',  't_2pi_mw_all_x', 'mw_wait']
