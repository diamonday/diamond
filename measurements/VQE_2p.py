...
from hardware.waveform import Sin, Sequence, Idle
...
class VQE_2p(Pulsed):
    ...
    |
    |
    |           Devices: 2smiq for MW and AWG610 for RF
    |
    ...
    saveWaitTime = Range(
        ...
        high=1440,
        value=60,
        desc="time interval of auto save",
        label="save every [min]",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    auto_save_dir = String(
        "D:/Data/2023/VQE_default",
        desc="auto save dir",
        label="auto save dir",
        mode="text",
    )
    ...
    one_round_time = Range(
        low=1,
        high=1e9,
        value=60,
        desc="sequence length for one round",
        label="sequence length[ns]",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    ...
    mw_wait = Range(
        ...
        high=5000000.0,
        value=400,
        desc="wait time after rf before mw",
        label="mw wait[ns]",
        mode="text",
        auto_set=False,
        enter_set=True,
    ...
    t_2pi_mw_all_y = Range(
        low=1.0,
        high=100000.0,
        value=1000.0,
        desc="rabi period 2pi pulse length for mw_all_y",
        label="2pi t mw_all_y[ns]",
        mode="text",
        auto_set=False,
        enter_set=True,
    )

    t_2pi_mw_all_x = Range(
        low=1.0,
        high=100000.0,
        value=1000.0,
        desc="rabi period 2pi pulse length for mw_all_x",
        label="2pi t mw_all_x[ns]",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    ...
    freq_rf_12 = Range(
        ...
        high=2.6e9,
        value=2.940e6,
        desc="radio frequency of energy gap between 1 and 2",
        label="frequency rf_12 [Hz]",
        mode="text",
        auto_set=False,
        enter_set=True,
        editor=TextEditor(
    ...
    amp_rf_12 = Range(
        ...
        high=1.0,
        value=1,
        desc="amp of radiowave of energy gap between 1 and 2",
        label="amp rf_12[500mVpp]",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    t_2pi_rf_12 = Range(
        ...
        high=5000000.0,
        value=200.0e3,
        desc="rabi period 2pi pulse length for rf_12",
        label="2pi t rf_12[ns]",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    ...
    """heating"""
    freq_awg_rf_heating = Range(
        low=1,
        high=2.6e9,
        value=2.5e6,
        desc="AWG_rf frequency heating",
        label="freq heating[Hz]",
        mode="text",
        auto_set=False,
        enter_set=True,
        editor=TextEditor(
        ...
        try:
            if self.auto_save == True:
                if self.state == "run":
                    ...
                    dir2 = "/vqe_%03.3f_%03.3f_%03.3f_%03.3f_%imin_%i.pys" % (
                        self.rot_n_y,
                        self.rot_n_x,
                        self.rot_e_y,
                        self.rot_e_x,
    ...
    def _auto_save_changed(self, new):
        ...
        if new:
            self.cron_event = CronEvent(
                self.auto_save_func, min=list(range(0, 1440, self.saveWaitTime))
            )
    ...
    def generate_heating_time(self):
        ...
        "Tomo pi13"
        heating_time.append(
            max_time - hard_pi_t - t_rot_rf - mw_wait - t_rot_e_y - t_rot_e_x - mw_wait
        )
    ...
    def generate_sequence(self):
        ...
        if mw_pi > mw_pi_2:
            hard_pi_seq = [(["mw_13", "mw_24"], mw_pi_2), (["mw_13"], mw_pi - mw_pi_2)]
            hard_pi_t = mw_pi
        else:
            hard_pi_seq = [(["mw_13", "mw_24"], mw_pi), (["mw_24"], mw_pi_2 - mw_pi)]
            hard_pi_t = mw_pi_2
        ...
        nondiag_sequences[0].append((["mw_all_x"], t_rot_e_x + hard_pi_2_t))
        nondiag_sequences[0].append(([], mw_wait))
        ...
        nondiag_sequences[1].append((["mw_all_x"], t_rot_e_x + hard_pi_2_t))
        nondiag_sequences[1].append(([], mw_wait))
        ...
        for j in range(12):
            ...
            sequence.append(
                (["aom"], (576 + 512) * 1e9 / self.sampling_rf + 450 + 4 * mw_wait)
            )  # for trigger dead time of awg610(for awg520, we can ignore it if we set the sampling rate as 1e9), +2000 is for 2 wait of mw between rf
    ...
    def VQE_wfm_rf(self, j):
        ...
        carry_wait = Idle(0)
        carry_wait.duration = int(
            mw_wait_rf
        )  # wait for the mw to finish, add to before and after mw
        ...
        pi2y_rf_p = [
            Sin(
                duration_pi2y_rf, freq=freq_rf_34, amp=self.amp_rf_34, phase=np.pi / 2.0
            )
        ]
        pi2y_rf_m = [
            Sin(
        ...
        pi_34_rf = [
            Sin(duration_pi_34_rf, freq=freq_rf_34, amp=self.amp_rf_34, phase=0)
        ]

        duration_pi_12_rf = int(int((t_2pi_rf_12 / 2) / T_rf_12) * T_rf_12)
        pi_12_rf = [
            Sin(duration_pi_12_rf, freq=freq_rf_12, amp=self.amp_rf_12, phase=0)
        ]

        rot_ny_duration = int(int((self.rot_n_y * t_2pi_rf_34) / T_rf_34) * T_rf_34)
        rot_nx_duration = int(int((self.rot_n_x * t_2pi_rf_34) / T_rf_34) * T_rf_34)
        ...
        t_makeup1 = (
            hard_pi_t_rft
            + self.rot_n_y * t_2pi_rf_34
            + self.rot_n_x * t_2pi_rf_34
            - carry_hard_pi.duration
            - rot_ny_duration
            - 1
            - rot_nx_duration
        )  # 1 is for zero_rf in pulse_rot
        carry_makeup1.duration = int(t_makeup1)

        carry_makeup2 = Idle(0)
        t_makeup2 = (
            2 * (mw_wait_rf - carry_wait.duration)
            + t_mw_rot
            - carry_mw_rot.duration
            + hard_pi_2_t_rft
            - carry_hard_pi_2.duration
            + t_2pi_rf_34 / 4
            - duration_pi2y_rf
        )
        ...
        """consider rf start from 0 for vqe waveform 8 and 9"""
        # besause the first rf pulse is a y pulse, to start from zero, carry should be N + 0.25
        carry_hard_pi_period1 = Idle(0)
        carry_hard_pi_period1.duration = int(
            (int(carry_hard_pi.duration / T_rf_34) + 1.25) * T_rf_34
        )

        # besause this rf pulse is a x pulse, to start from zero, carry should be N
        carry_makeup_period2 = Idle(0)
        t_before_rot_nx = carry_hard_pi_period1.duration + rot_ny.duration
        carry_makeup_period2.duration = int(T_rf_34 - (t_before_rot_nx % T_rf_34))

        # besause this rf pulse is a y pulse, to start from zero, carry should be N + 0.25
        carry_makeup_period3 = Idle(0)
        t_before_pi2_rf = (
            carry_hard_pi_period1.duration
            + rot_ny.duration
            + carry_makeup_period2.duration
            + rot_nx.duration
            + carry_makeup1.duration
            + carry_wait.duration
            + carry_mw_rot.duration
            + carry_wait.duration
            + carry_hard_pi_2.duration
        )
        carry_makeup_period3.duration = int(
            1.25 * T_rf_34 - (t_before_pi2_rf % T_rf_34)
        )

        pulse_rot = [
            rot_ny,
            carry_makeup_period2,
            rot_nx,
            carry_makeup1,
            carry_wait,
            carry_mw_rot,
        ...
        if j == 0:
            ...
            "N2"
            pulse_sequence_rf = (
                [zero_rf] + pi_12_rf + [zero_rf] + [rf_heating] + [zero_rf]
            )
            name_rf = "VQE_N2"
            wave_rf = Waveform610(
                name_rf,
            ...
            "N4"
            pulse_sequence_rf = (
                [carry_hard_pi] + pi_34_rf + [zero_rf] + [rf_heating] + [zero_rf]
            )
            name_rf = "VQE_N4"
            wave_rf = Waveform610(
                name_rf,
            ...
            "Tomo 0"
            pulse_sequence_rf = (
                [carry_hard_pi] + pulse_rot + [carry_pi13] + [rf_heating] + [zero_rf]
            )
            name_rf = "VQE_0"
            wave_rf = Waveform610(
                name_rf,
            ...
            "Tomo pi13"
            pulse_sequence_rf = (
                [carry_hard_pi] + pulse_rot + [zero_rf] + [rf_heating] + [zero_rf]
            )
            name_rf = "VQE_pi13"
            wave_rf = Waveform610(
                name_rf,
            ...
            "Tomo Pi"
            pulse_sequence_rf = (
                [carry_hard_pi] + pulse_rot + [carry_pi24] + [rf_heating] + [zero_rf]
            )
            name_rf = "VQE_Pi"
            wave_rf = Waveform610(
                name_rf,
            ...
            "rho23 +pi/2"
            pulse_sequence_rf = (
                [carry_hard_pi_period1]
                + pulse_rot
                + [carry_hard_pi_2]
                + [carry_makeup_period3]
                + pi2y_rf_p
                + [carry_makeup2, carry_wait, carry_pi13, carry_wait]
                + [rf_heating]
                + [zero_rf]
            )
            name_rf = "VQE_rho23_p"
            ...
            pulse_sequence_rf = (
                ...
                + [zero_rf]
                + pi2y_rf_p
                + [carry_makeup3, carry_wait, carry_pi13, carry_wait]
                + [rf_heating]
                + [zero_rf]
            )
            name_rf = "VQE_rho14_p"
    ...
    def compile_waveforms(self):
        ...
        name_rf = "VQE_nothing"
        wave_rf = Waveform610(
            name_rf, pulse_sequence_rf, offset=0, file_type=0, sampling=self.sampling_rf
        )  # 0 for wfm file and 1 for pat file
    ...
    def _create_line_plot(self):
        ...
        self.index_range = DataRange1D(
            self.x_data, low_setting=-0.5, high_setting=11.5, stretch_data=False
        )
        self.index_mapper = LinearMapper(range=self.index_range)
        self.value_range = DataRange1D(
            self.y1_data, low_setting="auto", high_setting="auto", stretch_data=False
        )
        ...
        self.plot1 = BarPlot(
            ...
            line_color="black",
            orientation="h",
            fill_color=tuple(COLOR_PALETTE[5]),
            bar_width=1,
            antialias=False,
            padding=8,
            padding_left=64,
        ...
        self.x_axis = LabelAxis(
            labels=_label,
            positions=_label_index,
            component=self.plot1,
            orientation="bottom",
                ...
                VGroup(
                    ...
                    HGroup(
                        HGroup(
                            ...
                            Item(
                                "t_2pi_mw_13", width=-80, enabled_when='state != "run"'
                            ),
                            Item(
                                "splitting",
                                editor=TextEditor(
