...
from traits.api import Trait, Instance, Property, Float, Range,\
                       Bool, Array, Str, Enum, Button, on_trait_change, cached_property
from traitsui.api import View, Item, HGroup, VGroup, EnumEditor, TextEditor, Action, Menu, MenuBar

from enable.api import ComponentEditor
from chaco.api import HPlotContainer, Plot, CMapImagePlot, ArrayPlotData, DataRange1D,\
                    jet, ColorBar, LinearMapper, DataLabel, PlotLabel
import chaco.api
...
class Confocal(ManagedJob, GetSetItemsMixin):
    ...
    z = Range(low=scanner.getZRange()[0], high=scanner.getZRange()[1], value=0, desc='z [micron]', label='z [micron]', mode='slider')
    # scan data
    X = Array()
    Y = Array()
    image = Array()
    ...
    constant_axis = Enum('z', 'x', 'y', label='constant axis', desc='axis that is not scanned when acquiring an image', editor=EnumEditor(values={'x':'1:x','y':'2:y','z':'3:z',},cols=3),)
    ...
    def __init__(self, scanner):
        ...
        self.labels = {}   # define a empty dictionary (global variable) called labels here
    ...
    def _set_scanner_position(self):
        if self.state != 'run':   # only when the scanning process is not running, we can reset the confocal point.
    ...
    def _run(self):
        ...
        try:
            ...
            # scan the image in the y direction with a set of horizontal lines
                ...
                if i%2 != 0 and self.bidirectional:
                    self.image[i,:] = c[-1::-1]   # i means the ith row(first dimension i)
