...
from tools.utility import GetSetSaveImageHandler, GetSetItemsMixin
...
class Confocal(ManagedJob, GetSetItemsMixin):
    ...
    # scanner position
    x = Range(
        low=scanner.getXRange()[0],
        high=scanner.getXRange()[1],
        value=0.5 * (scanner.getXRange()[0] + scanner.getXRange()[1]),
        desc="x [micron]",
        label="x [micron]",
        mode="slider",
    )
    y = Range(
        low=scanner.getYRange()[0],
        high=scanner.getYRange()[1],
        value=0.5 * (scanner.getYRange()[0] + scanner.getYRange()[1]),
        desc="y [micron]",
        label="y [micron]",
        mode="slider",
    )
    z = Range(
        low=scanner.getZRange()[0],
        high=scanner.getZRange()[1],
        value=0,
        desc="z [micron]",
        label="z [micron]",
        mode="slider",
    ...
    resolution = Range(
        ...
        high=1000,
        value=100,
        desc="Number of point in long direction",
        label="resolution",
        auto_set=False,
        enter_set=True,
    )
    ...
    return_speed = Range(
        ...
        high=100.0,
        value=10.0,
        desc="Multiplier for return speed of Scanner if mode is monodirectional",
        label="return speed",
        mode="text",
        auto_set=False,
        enter_set=True,
    ...
    def __init__(self, scanner):
        ...
        self.X = numpy.linspace(
            scanner.getXRange()[0], scanner.getXRange()[-1], self.resolution + 1
        )
        self.Y = numpy.linspace(
            scanner.getYRange()[0], scanner.getYRange()[-1], self.resolution + 1
        )
        # you will get a array of total length 101 if you choose the resolution to be 100
        self.image = numpy.zeros((len(self.X), len(self.Y)))
        # Y rows and X columns matrix to store the image counts data for plotting
        self._create_plot()
        # every time we need to initialize, we will call the _create_plot() function defined below.
        self.figure.index_range.on_trait_change(
            self.update_axis_li, "_low_value", dispatch="ui"
        )
        self.figure.index_range.on_trait_change(
            self.update_axis_hi, "_high_value", dispatch="ui"
        )
        self.figure.value_range.on_trait_change(
            self.update_axis_lv, "_low_value", dispatch="ui"
        )
        self.figure.value_range.on_trait_change(
            self.update_axis_hv, "_high_value", dispatch="ui"
        )
        self.zoom.on_trait_change(self.check_zoom, "box", dispatch="ui")
        # figure and zoom are defined in the begining of this class as global variables, here called as self.figure and self.zoom
        # update_axis_li() method, defined below, is called whenever the _low_value attribute of the figure.index_range is modified.
        # Dynamically, by calling on_trait_change() to establish (or remove) change notification handlers.
        self.on_trait_change(self.set_mesh_and_aspect_ratio, "X,Y", dispatch="ui")
        # set_mesh_and_aspect_ratio() method is called whenever the dimension of the image is changed(X,Y)
        self.on_trait_change(self.update_image_plot, "image", dispatch="ui")
        # update_image_plot() method is called whenever the image data is modified.
        ...
        # any change to the current_position attribute on the self.cursor object will result in the same value
        # being assigned to the cursor_position attribute
        # any change to the high_setting attribute on the self.scan_plot.value_range object will result in the same value
        # being assigned to the thresh_high attribute
        self.on_trait_change(
            self.scan_plot.request_redraw, "thresh_high", dispatch="ui"
        )
        self.on_trait_change(self.scan_plot.request_redraw, "thresh_low", dispatch="ui")
        # request_redraw() method is called whenever the thresh_high or thresh_low is modified.
        self.history = History(length=10)  # contain only 10 confocal images at most
        self.history.put(
            self.copy_items(
                ["constant_axis", "X", "Y", "image", "z_label_text", "resolution"]
            )
        )
        self.labels = (
            {}
        )  # define a empty dictionary (global variable) called labels here
        self.label_list = []  # an empty array

    # scanner position
    # Statically, by decorating methods on the class with the @on_trait_change decorator
    # to indicate that they handle notification for specified attributes.
    # _set_scanner_position() method is called whenever the 'x,y,z' is modified.
    @on_trait_change("x,y,z")
    def _set_scanner_position(self):
        if (
            self.state != "run"
        ):  # only when the scanning process is not running, we can reset the confocal point.
    ...
    def _get_cursor_position(self):
        if self.constant_axis == "x":
            ...
            # Think of the vertical little stripe   rearrange the horizontal z to the position (x,y)
        elif self.constant_axis == "y":
            return self.x, self.z
        elif self.constant_axis == "z":
            return self.x, self.y

    # @cached_property is a decorator for the Python language itself
    # this means call this _get_cursor_position() method twice won't change the readout cursor position.
    # Why doesn't the value of self._get_cursor_position() change? Because it's a cached property!

    def _set_cursor_position(self, position):
        if self.constant_axis == "x":
            self.z, self.y = position
            # Think of the vertical little stripe   the position's x is real Z(self.z), y is real Y.
        elif self.constant_axis == "y":
    ...
    def _run(self):
        """Acquire a scan"""
        try:
            # no need to generate multiple z slices or the constant axis is x or y (scan an image of the side view)
            self.state = "run"
            self.update_mesh()
            # redefine the vertical direction as the Y and horizontal direction as X (even though actually they may not be Y or X)
            X = self.X
            Y = self.Y
            XP = X[::-1]  # array reversed
            self.image = numpy.zeros((len(Y), len(X)))
            # creat a array with Y rows (first dimension, y coordinates) and X columns (second dimension, x coordinates)
            # to contain the acquired counts data of each scanned point
            time.sleep(0.5)

            if self.constant_axis == "x":
                # scan yz plane, look from the left
                const = self.x * numpy.ones(X.shape)
                # constitute the yz plane's constant x coordinates
                scanner.setPosition(self.x, Y[0], X[0])
                # redefine actual z as X and y as Y. think of the vertical stripe you scanned when choose the z constant
            elif self.constant_axis == "y":
                # scan xz plane, look from the top
                const = self.y * numpy.ones(X.shape)
                scanner.setPosition(X[0], self.y, Y[0])
            elif self.constant_axis == "z":
                # actually this may happen, as long as we do not scan multiple z images (the condition is or for the if above)
                const = self.z * numpy.ones(X.shape)
                scanner.setPosition(X[0], Y[0], self.z)
                # the redefined XY is the actual xy coordinate
            time.sleep(0.5)

            for i, y in enumerate(Y):
                # scan the image in the y direction with a set of horizontal lines
                if threading.current_thread().stop_request.isSet():
                    break
                # once the stop button is pressed, stop the scan process
                if i % 2 != 0 and self.bidirectional:
                    XL = XP  # XL means XLine
                    # if the bidirectional scan mode is set, the scan direction is reversed for the odd number line
                    # since the first line is i = 0, actually the even number lines are scanned in the reversed direction
                    # XP is the array reversed above (scan from right to left)
                else:
                    XL = X

                YL = y * numpy.ones(X.shape)  # Chen Yang
                # this should outside the else, otherwise the 2nd line will be the same as the 1st one,just reverse direction

                if self.constant_axis == "x":
                    Line = numpy.vstack((const, YL, XL))
                    # stack as 3 rows, first row is x, then y and z.
                elif self.constant_axis == "y":
                ...
                if self.bidirectional:
                    # scan like a zigzag trace
                    c = scanner.scanLine(Line, self.seconds_per_point) / 1.0e3
                    # the counts is measured in kcounts in the confocal map
                    # c is the temperal variable used to contain the counts of the line just scanned.
                else:
                    c = (
                        scanner.scanLine(
                            Line, self.seconds_per_point, return_speed=self.return_speed
                        )
                        / 1.0e3
                    )
                    # return to the left edge with 10 (return speed) times speed

                if i % 2 != 0 and self.bidirectional:
                    self.image[i, :] = c[
                        -1::-1
                    ]  # i means the ith row(first dimension i)
                    # put the counts of the line just acquired to the big matrix to plot, reversed the counts if needed.
                    # the [-1::-1] is the same as [::-1],[start:end:step]
                else:
                    self.image[i, :] = c[:]
                self.trait_property_changed("image", self.image)
                # update the image data for plotting after each line scan, that's why the line's vertical coordinate is increasing.
                if self.constant_axis == "x":
                    self.z_label_text = "x:%.2f" % self.x
                    # put the thing (here the value of variable self.x) after second % to the first % with precision .2f
                elif self.constant_axis == "y":
                    self.z_label_text = "y:%.2f" % self.y
                elif self.constant_axis == "z":
                    self.z_label_text = "z:%.2f" % self.z
                # update the text at the left bottom corner to indicate the slice position.
    ...
    def _create_plot(self):
        ...
        cursor = CursorTool2D(
            image, drag_button="left", color="white", marker_size=1.0, line_width=1.0
        )
    ...
    def update_mesh(self):
        ...
        if (x2 - x1) >= (y2 - y1):
            ...
            self.Y = numpy.linspace(
                y1, y2, int(self.resolution * round((y2 - y1), 2) / round((x2 - x1), 2))
            )
        else:
            self.Y = numpy.linspace(y1, y2, self.resolution)
            self.X = numpy.linspace(
                x1, x2, int(self.resolution * round((x2 - x1), 2) / round((y2 - y1), 2))
            )
    ...
    traits_view = View(
        VGroup(
            ...
            Item(
                "x",
                enabled_when='state != "run" or (state == "run" and constant_axis == "x")',
            ),
            Item(
                "y",
                enabled_when='state != "run" or (state == "run" and constant_axis == "y")',
            ),
            Item(
                "z",
                enabled_when='state != "run" or (state == "run" and constant_axis == "z")',
            ),
        ),
        menubar=MenuBar(
            Menu(
                Action(action="save_image", name="Save Image (.png)"),
