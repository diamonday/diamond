...
class Pulsed_SingleShot(ManagedJob, GetSetItemsMixin):
	...
	# Swap and Readout
	swap_mw_freq = Range(low=1e9, high=10e9, value=2.87e9, desc="MW Frequency", label="MW Freq [Hz]", mode='text', auto_set=False, enter_set=True)
	swap_mw_amp = Range(low=0.0, high=1.0, value=0.1, desc="MW Amplitude", label="MW Amplitude", mode='text', auto_set=False, enter_set=True)
	swap_mw_t_pi = Range(low=0.0, high=10000.0, value=100.0, desc="MW Pi", label="MW pi [ns]", mode='text', auto_set=False, enter_set=True)
	
	swap_rf_freq = Range(low=1e3, high=100e6, value=3e6, desc="RF Frequency", label="RF Freq [Hz]", mode='text', auto_set=False, enter_set=True)
	swap_rf_amp = Range(low=0.0, high=1.0, value=0.1, desc="RF Power", label="RF Amplitude", mode='text', auto_set=False, enter_set=True)
	swap_rf_t_pi = Range(low=0.0, high=1e9, value=30000, desc="RF Pi", label="RF pi [ns]", mode='text', auto_set=False, enter_set=True)
	...
	read_end_pos = Range(low=300.0, desc="Read End [ns]", label="Read End [ns]", mode='text', auto_set=False, enter_set=True)
	...
	record_length = Range(low=0,  high=10000, value=350, desc='Record Length', label='Record Length [ns]', mode='text', auto_set=False, enter_set=True)
	bin_width = Range(low=0.1, high=1000., value=10.0, desc='Bin Width', label='Bin Width [ns]', mode='text', auto_set=False, enter_set=True)
...
#	mix_t = Range(low=0.0, high=1e8, desc="Mixing Time After Readout", label="Mix Time [ns]")
	...
	get_set_items.extend([
		'swap_mw_freq', 'swap_mw_amp', 'swap_mw_t_pi', 'swap_rf_freq', 'swap_rf_amp', 'swap_rf_t_pi',
	...
	get_set_items.extend(['lineplot_dark_threshold', 'lineplot_bright_threshold', 'lineplot_enhancedata'])
	...
	get_set_items.extend(['runs', 'time_bins', 'datapoints', 'data_bins', 'count_data', 'spin_state', 'enhanced_data', 'sequence', 'sequence_time', 'keep_data'])
	...
	@on_trait_change("count_data, lineplot_dark_threshold, lineplot_bright_threshold, lineplot_enhancedata")
	...
	def apply_parameters(self):
		...
		if self.keep_data and np.all(datapoints == self.datapoints) and np.all(self.data_bins == data_bins) and np.all(time_bins == self.time_bins):
			pass
		else:
			self.run_time = 0.0
			self.runs = 0
			...
			self.count_data = np.zeros((len(datapoints) * (1 + self.dual), data_bins.size + 2), dtype=np.int32)
	...
	def generate_sequence_swap(self, index, point):
		_sequence = [
			...
			(tuple(), self.awg_trigger_delay + self.swap_mw_t_pi * 2 + self.swap_rf_t_pi),
	...
	def generate_waveform_swap(self, index, point):
		...
		self.awg_sequence.seqBox[self.measurement_name].writeStep(
			(mw_wavename3, "Waveform"), (rf_wavename3, "Waveform"),
			WaitInput="None",
			Repeat="Once", 
			EventJumpInput="None",
			GoTo="Next"
	...
	def generate_waveform_readout(self, index, point):
		...
		t_pi = self.swap_mw_t_pi * (sampling / 1e9)
	...
	def generate_sequence(self):
		...
		for ind, dp in enumerate(self.datapoints):
			sequence += self.generate_sequence_measurement(ind, dp, 1) + _swap_seq + _read_seq + _mix_seq

		if self.dual:
			_len = len(self.datapoints)
			for ind, dp in enumerate(self.datapoints):
				sequence += self.generate_sequence_measurement(ind + _len, dp, 2) + _swap_seq + _read_seq + _mix_seq
	...
	def _run(self):
		...
		try:
			...
			while self.run_time < self.stop_time:
				...
				_data = _data.reshape(len(self.datapoints) * (1 + self.dual), self.read_num).sum(1)
...
class Rabi(Pulsed_SingleShot):
	...
	datapoints_selector = Instance(DataPointsSelector, factory=LinearPoints, args=((0.0, 1e6), "ns"))
	...
	mw_amp = Range(low=0.0, high=1.0, value=0.1, desc="MW Amplitude", label="MW Amplitude", mode='text', auto_set=False, enter_set=True)
