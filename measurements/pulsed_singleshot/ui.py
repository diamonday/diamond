...
class LinePlot(HasTraits):
    ...
    def __init__(self, dual=False, x_title="Tau [ns]", y_title="Signal [a.u]"):
        ...
        self.data = ArrayPlotData(
            x=np.array((0, 1)), y1=np.array((0, 0)), y2=np.array((0, 0))
        )

        self.active_plot = []

        self.add_trait(
            "plot", Plot(self.data, padding=8, padding_left=64, padding_bottom=36)
        )

        self.plot.plot(
            ("x", "y1"), color=color_map["y1"], line_width=2, id="0", name="y1"
        )
        self.active_plot.append("y1")
        if self.dual:
            self.plot.plot(
                ("x", "y2"), color=color_map["y2"], line_width=2, id="0", name="y2"
            )
        ...
        if name == "x":
            ...
            if isinstance(data, (list, np.ndarray)):
                ...
                if name not in self.active_plot:
                    ...
                    self.plot.plot(
                        ("x", name), color=_color, line_width=2, id="0", name=name
                    )
