...
class LinearPoints(DataPointsSelector):
	...
	def __init__(self, range, unit, **kwargs):
		...
		self.add_trait("start", Range(low=_lower, high=_upper, label="Start [%s]" % unit, mode='text', auto_set=False, enter_set=True, **kwargs))
		self.add_trait("end", Range(low=_lower, high=_upper, label="End [%s]" % unit, mode='text', auto_set=False, enter_set=True, **kwargs))
		self.add_trait("delta", Range(low=_type_low(0), high=_upper, label="Delta [%s]" % unit, mode='text', auto_set=False, enter_set=True, **kwargs))
