...
class Pulsed_SingleShot_PS(ManagedJob, GetSetItemsMixin):
    ...
    preread_num = Range(
        ...
        high=100000,
        value=3000,
        desc="Number of Preread Readout Repetitions",
        label="Preread Number",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    preread_count_low = Range(
        low=0,
        value=200,
        high=1000,
        desc="Preread Min Count",
        label="Preread Min Count",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    preread_count_high = Range(
        low=100,
        value=400,
        high=2000,
        desc="Preread Max Count",
        label="Preread Max Count",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    preread_count_delta = Range(
        low=1,
        value=5,
        hight=100,
        desc="Preread Delta Count",
        label="Preread Delta Count",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    ...
    save_cross_correlation = Bool(
        False, desc="Save Cross Correlation Data", label="Save Cross Correlation Data"
    )
    ...
    # Visualization
    lineplot_selection_dark_threshold = Range(
        low=0,
        high=1000,
        value=300,
        label="Selection Dark Threshold",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    ...
    get_set_items.extend(
        [
            "lineplot_selection_dark_threshold",
            "lineplot_dark_threshold",
            "lineplot_bright_threshold",
            "lineplot_enhancedata",
        ]
    )

    histogram_preread = Bool(desc="Preread Data", label="Preread Data")
    histogram_selection_dark_threshold = Range(
        low=0,
        high=1000,
        value=300,
        label="Selection Dark Threshold",
    ...
    get_set_items.extend(
        [
            "histogram_preread",
            "histogram_selection_dark_threshold",
            "histogram_index",
            "histogram_datapoint",
        ]
    )
    ...
    @on_trait_change(
        "count_data, lineplot_selection_dark_threshold, lineplot_dark_threshold, lineplot_bright_threshold, lineplot_enhancedata"
    )
    def update_spin_state(self):
        ...
        if (
            self.lineplot_selection_dark_threshold < self.preread_count_low
            or self.lineplot_selection_dark_threshold > self.preread_count_high
        ):
        ...
        if self.lineplot_dark_threshold < self.read_count_low:
            ...
            _low_count = (
                self.count_data[:, 1:_selection_thres, 1:_thres]
                .sum(2, dtype=np.float)
                .sum(1)
            )
        ...
        if self.lineplot_bright_threshold > self.read_count_high:
            ...
            _high_count = (
                self.count_data[:, 1:_selection_thres, _thres:-1]
                .sum(2, dtype=np.float)
                .sum(1)
            )
    ...
    @on_trait_change(
        "histogram_preread, histogram_selection_dark_threshold, histogram_index, selection_bins, data_bins, datapoints"
    )
    def update_histogram(self):
        if (
            len(self.datapoints) == 0
            or len(self.selection_bins) == 0
            or len(self.data_bins) == 0
        ):
            return
        ...
        if self.histogram_preread:
            ...
            if self.runs and len(self.count_data):
                _selection_thres = (
                    int(
                        (
                            self.histogram_selection_dark_threshold
                            - self.preread_count_low
                        )
                        // self.preread_count_delta
                    )
                    + 1
                ...
                self.histogram.update_data(
                    "y1", self.count_data[_ind][1:_selection_thres].sum(0)
                )
                if self.dual:
                    self.histogram.update_data(
                        "y2", self.count_data[_ind + _len][1:_selection_thres].sum(0)
                    )
    ...
    def update_count_data_change(self):
        ...
        if self.histogram_preread:
            ...
            _selection_thres = (
                int(
                    (self.histogram_selection_dark_threshold - self.preread_count_low)
                    // self.preread_count_delta
                )
                + 1
            )
    ...
    def apply_parameters(self):
        ...
        selection_bins = np.arange(
            self.preread_count_low,
            self.preread_count_high + self.preread_count_delta,
            self.preread_count_delta,
        )
        data_bins = np.arange(
            self.read_count_low,
        ...
        depth = int(
            (self.preread_num + self.read_num) * len(datapoints) * (1 + self.dual)
        )
        self.depth = depth
        ...
        if (
            ...
            and np.all(self.selection_bins == selection_bins)
            ...
            self.count_data = np.zeros(
                (
                    len(datapoints) * (1 + self.dual),
                    selection_bins.size + 2,
                    data_bins.size + 2,
                ),
                dtype=np.int32,
            )
            ...
            if self.save_cross_correlation:
                self.cross_correlation_data = np.zeros(
                    (
                        len(datapoints) * (1 + self.dual) - 1,
                        data_bins.size + 2,
                        selection_bins.size + 2,
                    ),
                    dtype=np.int32,
    ...
    def generate_waveform_preread(self, index, point):
        ...
        self.init_mw_t_pi * (sampling / 1e9)
            ...
            while self.run_time < self.stop_time:
                ...
                if _iter == 50001:
                    logging.getLogger().warn(
                        "Singleshot Post Selection Measurement Timed out."
                    )
                    _exit_reason = 3
                ...
                if self.save_raw_data:
                    np.save(
                        os.path.join(self.raw_data_path, "%i.npy" % self.runs), _data
                    )
                ...
                for i in range(0, len(self.datapoints) * (1 + self.dual)):
                    ...
                    _preread_sum = _data[
                        i * _read_per_dp : i * _read_per_dp + self.preread_num
                    ].sum()
                    _read_sum = _data[
                        i * _read_per_dp + self.preread_num : (i + 1) * _read_per_dp
                    ].sum()
                    ...
                    if _preread_sum < self.preread_count_low:
                        ...
                        _preread_ind = (
                            _preread_sum - self.preread_count_low
                        ) // self.preread_count_delta + 1
                    ...
                    if _read_sum < self.read_count_low:
                        ...
                        _ind = (
                            _read_sum - self.read_count_low
                        ) // self.read_count_delta + 1
                ...
                if self.save_cross_correlation:
                    for i in range(0, len(self.datapoints) * (1 + self.dual) - 1):
                        ...
                        _preread_sum = _data[
                            (i + 1) * _read_per_dp : (i + 1) * _read_per_dp
                            + self.preread_num
                        ].sum()
                        _read_sum = _data[
                            i * _read_per_dp + self.preread_num : (i + 1) * _read_per_dp
...
class NuclearRabi(Pulsed_SingleShot_PS):
    ...
    datapoints_selector = Instance(
        DataPointsSelector, factory=LinearPoints, args=((0.0, 1000000.0), "ns")
    )
...
class NuclearMagneticResonance(Pulsed_SingleShot_PS):
    ...
    datapoints_selector = Instance(
        DataPointsSelector, factory=LinearPoints, args=((0.0, 10e6), "Hz")
    )
    ...
    fit_threshold = Range(
        ...
        high=99.0,
        value=50.0,
        desc="Fit Threshold for detection of resonances [%]",
        label="Fit Threshold [%]",
        mode="text",
        auto_set=False,
        enter_set=True,
    ...
    fit_contrast = Float(0.0, desc="Contrast", label="Contrast")
    ...
    def fit_data(self):
        ...
        try:
            fit_result = fitting.fit_multiple_lorentzians(
                self.datapoints, self.spin_state, 1, threshold=self.fit_threshold / 100
            )
        ...
        if fit_result[0] != np.NaN:
            self.lineplot.update_data(
                "fit1", fitting.NLorentzians(*fit_result)(self.datapoints)
            )
