...
class QuantumNonDemolition(ManagedJob, GetSetItemsMixin):
    ...
    get_set_items.extend(
        ["runs", "data_bins", "count_data", "sequence", "sequence_time", "keep_data"]
    )
    ...
    def _run(self):
        ...
        try:
            ...
            _counter = self.time_tagger.Counter(
                self.tt_ch_apd, int(self.read_time * 1e12), self.read_runs_round
            )
            ...
            while self.run_time < self.stop_time:
                ...
                if self.save_time_trace:
                    _path = (
                        os.path.normpath(self.time_trace_path)
                        + "/"
                        + "MW_%i.npy" % self.runs
                    )
