"""
Auto focus tool with the function that it can trace the count of the max of the autofocus result.

remember to fotget count trace before first use this.
I don't know why but I think this bug will also happen when using drift function
"""
...
from tools.utility import GetSetItemsHandler, GetSetItemsMixin, warning
...
class AutoFocusTraceHandler(GetSetItemsHandler):
    ...
    def forget_count_trace(self, info):
        info.object.forget_count_trace()


class AutoFocusTrace(ManagedJob, GetSetItemsMixin):
    ...
    size_xy = Range(
        ...
        high=10.0,
        value=1.5,
        desc="Size of XY Scan",
        label="Size XY [micron]",
        mode="slider",
        auto_set=False,
        enter_set=True,
    )
    ...
    step_xy = Range(
        ...
        high=10.0,
        value=0.1,
        desc="Step of XY Scan",
        label="Step XY [micron]",
        mode="slider",
        auto_set=False,
        enter_set=True,
    )
    ...
    seconds_per_point_xy = Range(
        low=1e-3,
        high=10,
        value=0.05,
        desc="Seconds per point for XY Scan",
        label="seconds per point XY [s]",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    seconds_per_point_z = Range(
        low=1e-3,
        high=10,
        value=0.2,
        desc="Seconds per point for Z Scan",
        label="seconds per point Z  [s]",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    ...
    delay = Range(
        ...
        high=60.0,
        value=0.0,
        desc="Delay before starting auto focus",
        label="Delay [s]",
        mode="text",
        auto_set=False,
        enter_set=True,
    ...
    fit_method_xy = Enum(
        "Maximum", "Gaussian", desc="Fit Method for XY Scan", label="XY Fit Method"
    )
    fit_method_z = Enum(
        "Maximum", "Gaussian", desc="Fit Method for Z Scan", label="Z Fit Method"
    )

    X = Array(value=np.array((0.0, 1.0)))
    Y = Array(value=np.array((0.0, 1.0)))
    Z = Array(value=np.array((-1.0, 1.0)))

    data_xy = Array()
    data_z = Array(value=np.array((0, 0)))

    targets = Instance(
        {}.__class__, factory={}.__class__
    )  # Dict traits are no good for pickling, therefore we have to do it with an ordinary dictionary and take care about the notification manually
    target_list = Instance(
        list, factory=list, args=([None],)
    )  # list of targets that are selectable in current_target editor
    ...
    count_trace_time = Array(value=np.array((0,)))
    current_count_trace = Array(value=np.array((0, 0, 0)))
    ...
    remove_current_target_button = Button(
        label="Remove Current", desc="remove current target"
    )
    next_target_button = Button(
        label="Next Target", desc="switch to next available target"
    )
    ...
    plot_data_count_trace = Instance(ArrayPlotData)
    ...
    figure_count_trace = Instance(Plot, editor=ComponentEditor())
    image_plot = Instance(CMapImagePlot)

    def __init__(self, confocal):
        super(AutoFocusTrace, self).__init__()
        ...
        self.on_trait_change(
            self.update_plot_count_trace_value, "count_trace", dispatch="ui"
        )
        self.on_trait_change(
            self.update_plot_count_trace_index, "count_trace_time", dispatch="ui"
        )
    ...
    def next_target(self):
        ...
        if len(keys) == 0:
            ...
            self.current_target = keys[
                (keys.index(self.current_target) + 1) % len(keys)
            ]

    def _targets_changed(self, name, old, new):
        l = list(new.keys()) + [None]  # rebuild target_list for Enum trait
        l.sort()
        self.target_list = l
    ...
    def _draw_targets(self):
        ...
        c.remove_all_labels()
        c.show_labels = True
        for key, coordinates in list(self.targets.items()):
    ...
    def add_target(self, key, coordinates=None):
        ...
        )  # trigger event such that Enum is updated and Labels are redrawn
    ...
    def forget_count_trace(self):
        # set current_count_trace to 0 and clear plot
        # self.current_count_trace = np.array((0., 0., 0.))
        self.count_trace_time = np.array((time.time(),))
        self.count_trace = (self.current_count_trace,)
    ...
    def _run(self):
        ...
        try:
            ...
            if self.current_target is None:
                ...
                self.current_count_trace = np.array(
                    (self.data_z.max() / 1000.0, 0.0, 0.0)
                )
                self.count_trace = np.append(
                    self.count_trace, (self.current_count_trace,), axis=0
                )
                self.count_trace_time = np.append(self.count_trace_time, time.time())

                logging.getLogger().debug(
                    "Drift: %.2f, %.2f, %.2f" % tuple(self.current_drift)
                )
        finally:
            self.state = "idle"
    ...
    def focus(self):
        ...
        xmin = np.clip(
            xp - 0.5 * self.size_xy,
            scanner.getXRange()[0] + safety,
            scanner.getXRange()[1] - safety,
        )
        xmax = np.clip(
            xp + 0.5 * self.size_xy,
            scanner.getXRange()[0] + safety,
        ...
        ymin = np.clip(
            yp - 0.5 * self.size_xy,
            scanner.getYRange()[0] + safety,
            scanner.getYRange()[1] - safety,
        )
        ymax = np.clip(
            yp + 0.5 * self.size_xy,
            scanner.getYRange()[0] + safety,
        ...
        Z = np.hstack(
            (
                np.arange(zp, zp - 0.5 * self.size_z, -self.step_z),
    ...
    def undo(self):  # did't get the count_trace function here
    ...
    def _plot_data_count_trace_default(self):
        return ArrayPlotData(t=self.count_trace_time, x=self.count_trace[:, 0])
    ...
    def _figure_image_default(self):
        ...
        plot.aspect_ratio = 1
        plot.index_mapper.domain_limits = (
            scanner.getXRange()[0],
            scanner.getXRange()[1],
        )
        plot.value_mapper.domain_limits = (
            scanner.getYRange()[0],
            scanner.getYRange()[1],
        )
    ...
    def _figure_count_trace_default(self):
        plot = Plot(
            self.plot_data_count_trace,
            width=100,
            height=100,
            padding=8,
            padding_left=64,
        ...
        plot.plot(("t", "x"), type="line", color="blue", name="count")
        bottom_axis = PlotAxis(
            plot,
            orientation="bottom",
            tick_generator=ScalesTickGenerator(scale=CalendarScaleSystem()),
    ...
    def update_plot_drift_index(self):
        if len(self.drift_time) == 0:
            ...
            self.plot_data_drift.set_data("t", self.drift_time)

    def update_plot_count_trace_value(self):
        if len(self.count_trace) == 0:
            self.plot_data_count_trace.set_data("x", np.array(()))
        else:
            self.plot_data_count_trace.set_data("x", self.count_trace[:, 0])

    def update_plot_count_trace_index(self):
        if len(self.count_trace_time) == 0:
            self.plot_data_count_trace.set_data("t", np.array(()))
        else:
            self.plot_data_count_trace.set_data("t", self.count_trace_time)
    ...
    traits_view = View(
        VGroup(
            ...
            VSplit(
                ...
                Item("figure_count_trace", show_label=False, resizable=True),
            ),
        ),
        menubar=MenuBar(
            Menu(
            ...
            Menu(
                ...
                Action(action="forget_count_trace", name="Forget count_trace"),
                name="Target",
            ),
        ),
        title="Auto Focus with Trace",
        width=500,
        height=700,
        buttons=[],
        resizable=True,
