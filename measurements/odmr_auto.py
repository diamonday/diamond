...
from measurements.pulsed import Pulsed, find_laser_pulses
...
class ODMR(ManagedJob, GetSetItemsMixin):
    ...
    power = Range(low= -100., high=0., value= -30.0, desc='Power [dBm]', label='Power [dBm]', mode='text', auto_set=False, enter_set=True)
    frequency_begin = Range(low=1, high=3.3e9, value=2.8e9, desc='Start Frequency [Hz]', label='Begin [Hz]', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    frequency_end = Range(low=1, high=3.3e9, value=2.94e9, desc='Stop Frequency [Hz]', label='End [Hz]', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    frequency_delta = Range(low=1e-3, high=3.3e9, value=1.0e+6, desc='frequency step [Hz]', label='Delta [Hz]', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    ...
    frequency_delta_p = Range(low=1e-3, high=3.3e9, value=1.0e5, desc='frequency step Pmode[Hz]', label='Delta Pmode[Hz]', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    ...
    priority = Range(low=0, high=10, value=1, desc='priority of the job', label='priority', mode='text', auto_set=False, enter_set=True)
    ...
    focus_index = Range(low=0, high=1000, value=1, desc='The peak being focused on', label='focus index', auto_set=False, enter_set=True)
    ...
    frequency_size = Range(low=1, high=6.4e9, value=1.0e6, desc='Radius of the frequency list[Hz]', label='Frequency Size[Hz]', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    frequency_step = Range(low=1e-3, high=3.3e9, value=2.0e4, desc='Frequency step Pmode[Hz]', label='Delta Pmode[Hz]', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    ...
    randomize = Bool(True)
    randomize_interval = Range(low= 1, high=1000, value= 5, label='Randomize Interval [Runs]', evaluate=int, auto_set=False, enter_set=True)
    ...
    def _run(self):
        ...
        try:
            ...
            if self.pulsed:
                ...
                ha.PulseGenerator().Sequence(100 * [ (['B','laser', 'aom'], self.laser), (['B',], self.wait), (['A','B','mw_x'], self.t_pi) ])
            ...
            while self.run_time < self.stop_time:
                ...
                if self.randomize:
                    ...
                    for _old_ind, _new_ind in enumerate(_rand_mapping):
                        logging.getLogger().debug('Data Insertion Position: ' + str(_new_ind))
                        ...
                        for m in self.measurement_list:
                            if hasattr(m,'measurement'):
                                if hasattr(m.measurement,'frequency'):
                                    ...
                                    print(m.measurement, 'has no attribute called either "frequency" or "freq"')
                                ...
                                print(m, 'has no attribute called either "frequency" or "freq"')
    ...
    traits_view = View(
        VGroup(
            ...
            VGroup(
                ...
                Tabbed(
                    VGroup(
                        HGroup(
                            Item('power', height=.2, width= -40, enabled_when='state != "run" and pulsed == False')
                        ),
                        HGroup(
                            Item('frequency_begin', enabled_when='state != "run" and pulsed == False'),
                            Item('frequency_end', enabled_when='state != "run" and pulsed == False'),
                            Item('frequency_delta', enabled_when='state != "run" and pulsed == False'),
                        ),
                        label='Continuous'
                    ),
                    VGroup(
                        HGroup(
                            Item('power_p', enabled_when='state != "run" and pulsed == True'),
                            Item('t_pi', enabled_when='state != "run" and pulsed == True'),
                        ),
                        HGroup(
                            Item('frequency_begin_p', enabled_when='state != "run" and pulsed == True'),
                            Item('frequency_end_p', enabled_when='state != "run" and pulsed == True'),
                            Item('frequency_delta_p', enabled_when='state != "run" and pulsed == True'),
                        ),
                        label='Pulsed'
                    ),
                    VGroup(
                    ...
                    VGroup(
                        HGroup(
                            ...
                            Item('randomize_interval', enabled_when='state != "run" and randomize == True'),
                        ),
                        HGroup(
        ...
        title='ODMR', width=1000, height=800, buttons=[], resizable=True, handler=ODMRHandler
    )
    ...
    get_set_items = [
        ...
        'frequency_begin', 'frequency_end', 'frequency_delta',
        ...
        'randomize', 'randomize_interval',
        'focus_index','frequency_size','frequency_step','periodic_focus',
        '__doc__'
    ]
...
class ODMR_AWG(ManagedJob, GetSetItemsMixin):
    ...
    focus_interval = Range(low=1, high=6000, value=10, desc='Time interval between automatic focus events', label='Interval [m]', auto_set=False, enter_set=True)
    periodic_focus    = Bool(False, label='Periodic focusing')
    focus_index = Range(low=1, high=1000, value=1, desc='The peak being focused on', label='focus index', auto_set=False, enter_set=True)
    power_p = Range(low= -100., high=20., value= -20, desc='Power Pmode [dBm]', label='Power Pmode[dBm]', mode='text', auto_set=False, enter_set=True)
    frequency_size = Range(low=1, high=6.4e9, value=1.0e6, desc='Radius of the frequency list[Hz]', label='Frequency Size[Hz]', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    frequency_step = Range(low=1e-3, high=3.3e9, value=2.0e4, desc='Frequency step Pmode[Hz]', label='Delta Pmode[Hz]', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    ...
    trig_interval = Range(low=11.0, high=10000.0, value=20.0, label='Trig Interval', desc='Set the trigger interval for Atrigger and Btrigger of the AWG.', mode='text', auto_set=False, enter_set=True)
    sampling   = Range(low=50.0e3, high=1.0e9, value=1.0e9, label='sampling [Hz]', desc='Set the sampling rate of the AWG.', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    waves      = []
    
    amp1  = Range(low=0.0, high=1.0, value=1.0, desc='Waveform amplitude factor', label='wfm amp', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%.2f'))
    frequency_awg = Range(low=1, high=1e9, value=40e6, desc='AWG frequency', label='AWG frequency [Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
                    ...
                    if not np.isnan(fp):
                        for m in self.measurement_list:
                            if hasattr(m,'frequency'):
                                m.frequency = self.fit_frequencies[int(self.focus_index - 1)]
                            elif hasattr(m,'freq'):
                                print('awg')
    ...
    traits_view = View(
        ...
        title='ODMR (AWG)', width=1000, height=800, buttons=[], resizable=True, handler=ODMRHandler
    )
    ...
    get_set_items = [
        ...
        'fit_parameters', 'fit_contrast', 'fit_line_width', 'fit_frequencies',
        'perform_fit', 'run_time',
        'frequency_awg','phase_sin','amp1',
        'power_p', 'frequency_begin_p', 'frequency_end_p', 'frequency_delta_p',
        'laser', 'wait', 't_pi',
        'vpp1', 'vpp2', 'sampling','trig_interval',
        'seconds_per_point', 'stop_time', 'n_lines',
        'number_of_resonances', 'threshold',
        'randomize', 'randomize_interval',
        'focus_index','frequency_size','frequency_step','periodic_focus',
