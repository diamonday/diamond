...
from traits.api import HasTraits, Instance, Range, Bool, Array, on_trait_change
...
class CountTrace(HasTraits):
    ...
    def __init__(self, time_tagger, *args, **kwargs):
        ...
        self._stoppable_thread = StoppableThread(
            target=CountTrace._run, args=(weakref.proxy(self),)
        )
        self._stoppable_thread.start()

    def _create_counter(self):
        self._counter_0 = TimeTagger.Counter(
            self.time_tagger,
            0,
            int(self.seconds_per_point * 1e12),
            self.number_of_points,
        )
        self._counter_1 = TimeTagger.Counter(
            self.time_tagger,
            1,
            int(self.seconds_per_point * 1e12),
            self.number_of_points,
        )
        self._counter_2 = TimeTagger.Counter(
            self.time_tagger,
            2,
            int(self.seconds_per_point * 1e12),
            self.number_of_points,
        )
        self._counter_3 = TimeTagger.Counter(
            self.time_tagger,
            3,
            int(self.seconds_per_point * 1e12),
            self.number_of_points,
        )
        self._counter_4 = TimeTagger.Counter(
            self.time_tagger,
            4,
            int(self.seconds_per_point * 1e12),
            self.number_of_points,
        )
        self._counter_5 = TimeTagger.Counter(
            self.time_tagger,
            5,
            int(self.seconds_per_point * 1e12),
            self.number_of_points,
        )
        self._counter_6 = TimeTagger.Counter(
            self.time_tagger,
            6,
            int(self.seconds_per_point * 1e12),
            self.number_of_points,
        )
        self._counter_7 = TimeTagger.Counter(
            self.time_tagger,
            7,
            int(self.seconds_per_point * 1e12),
            self.number_of_points,
