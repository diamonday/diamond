...
class NuclearRabi_e0_rfmwrf( Pulsed ):
    ...
    t_pi_rf = Range(low=1., high=1.e9, value=200e3, desc='length of rf pi pulse [ns]', label='t pi rf [ns]', mode='text', auto_set=False, enter_set=True)
    ...
    get_set_items = Pulsed.get_set_items + [ 'freq_awg','freq_awg_rf', 'amp1', 'show_fit', 'fit_parameters', 'rabi_frequency', 'rabi_period',
        'rabi_offset', 'rabi_contrast', 'pi2', 'pi', 'pi32', 'phase_sin', 'mw_pi', 't_pi_rf'
    ]
    
    def __init__(self, pulse_generator, time_tagger, microwave, awg, awg_rf, **kwargs):
        self.dual = False
        super(NuclearRabi_e0_rfmwrf, self).__init__(pulse_generator, time_tagger, microwave, awg, awg_rf, **kwargs)
        self.show_fit = True
    ...
    def generate_sequence(self):
        ...
        sequence = [
            (['aom'], laser),
            (['aom'], 576 * 1e9 /self.sampling_rf + 450 ), #to be the same with other pulse
            ([ ], wait )
        ]
        ...
        for t in tau:
            ...
            sequence.append((['aom'], 576 * 1e9 /self.sampling_rf + 450 )) #for trigger dead time of awg610 and excite nulcear
            ...
            Tabbed(
                VGroup(
                    ...
                    HGroup(
                        ...
                        Item('t_pi_rf', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_func=lambda x:'%.2f'%x), width=-80),
