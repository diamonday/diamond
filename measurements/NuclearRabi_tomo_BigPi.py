...
class NuclearRabi_smiq_Nnorm( Pulsed ):
    ...
    get_set_items = Pulsed.get_set_items + [ 'freq_awg_rf', 'amp1_rf', 'show_fit', 'fit_parameters', 'rabi_frequency', 'rabi_period',
        'rabi_offset', 'rabi_contrast', 'pi2', 'pi', 'pi32',  'mw_pi', 'mw_pi_2',  'mw_pi_rf'
    ]
    
    def __init__(self, pulse_generator, time_tagger, microwave, microwave2, awg_rf, **kwargs):
        self.dual = False
        super(NuclearRabi_smiq_Nnorm, self).__init__(pulse_generator, time_tagger, microwave, microwave2, awg_rf, **kwargs)
        self.show_fit = True
        self.frequency2 = self.frequency
    ...
    def generate_sequence(self):
        ...
        sequence = [
            ([], max_time + trig_interval_rf + trig_delay_rf + mw_pi + mw_pi_2 - 100),
            (['aom'], laser),
            (['aom'], (576+120000) * 1e9 /self.sampling_rf + 450 ),
            ([], wait )
        ]
        
        "First 4 sequences to calibrate TOMO intial state count"
        'N1'
        sequence.append(([], max_time + trig_interval_rf + trig_delay_rf + mw_pi + mw_pi_2 + mw_pi_rf)) # the typical time between the laser illumination is : wait + max_time + trig_interval_rf + trig_delay_rf + mw_pi + mw_pi_2 + mw_pi_rf
        sequence.append((['laser', 'aom'], laser))
        sequence.append((['aom' ], (576+120000) * 1e9 /self.sampling_rf + 450 )) #for trigger dead time of awg610(for awg520, we can ignore it if we set the sampling rate as 1e9)
        sequence.append(([], wait))

        'N2'
        sequence.append(([], max_time+ mw_pi_2 - mw_pi))
        ...
        'N3'
        sequence.append(([], max_time + trig_interval_rf + trig_delay_rf + mw_pi_2 + mw_pi_rf))
        sequence.append((['A','mw_x'], mw_pi))
        sequence.append((['laser', 'aom'], laser))
        sequence.append((['aom' ], (576+120000) * 1e9 /self.sampling_rf + 450 )) #for trigger dead time of awg610(for awg520, we can ignore it if we set the sampling rate as 1e9)
        sequence.append(([], wait))
        ...
        'N4'
        sequence.append(([], max_time + mw_pi_2))
        sequence.append((['awg_rf'],  trig_interval_rf))
        sequence.append(([], trig_delay_rf))
        sequence.append(([], mw_pi_rf))
        sequence.append((['A','mw_x'], mw_pi))
        ...
        for t in tau:
            ...
            'Tomo 0'
            sequence.append(([], max_time - t + mw_pi_rf)) #consider (['sequence'], 100)
            sequence.append((['awg_rf'],  trig_interval_rf))
            sequence.append(([], trig_delay_rf))
            sequence.append((['A','mw_x'], mw_pi))
            sequence.append(([], t))
            ...
            'Tomo pi13'
            sequence.append(([], max_time - t + mw_pi_2 + mw_pi_rf)) #consider (['sequence'], 100)
            sequence.append((['awg_rf'],  trig_interval_rf))
            sequence.append(([], trig_delay_rf))
            sequence.append((['A','mw_x'], mw_pi))
            sequence.append(([], t))
            ...
            'Tomo pi34'
            sequence.append(([], max_time - t)) #consider (['sequence'], 100)
            sequence.append((['awg_rf'],  trig_interval_rf))
            sequence.append(([], trig_delay_rf))
            sequence.append((['A','mw_x'], mw_pi))
            sequence.append(([], t))
            ...
            'Tomo Pi'
            sequence.append(([], max_time - t - mw_pi + mw_pi_rf)) #consider (['sequence'], 100)
            sequence.append((['awg_rf'],  trig_interval_rf))
            sequence.append(([], trig_delay_rf))
            sequence.append((['A','mw_x'], mw_pi))
            sequence.append(([], t))
    ...
    def compile_waveforms(self):
        ...
        mw_pi_rf_rft = self.mw_pi_rf * self.sampling_rf / 1e9
        ...
        carry_pi13 = Idle(0)
        carry_pi13.duration = int(mw_pi_2_rft) 
        mw_rf_tomo_pi34 = Sin(0, freq=f1_rf, amp=self.amp1_rf, phase=phase_rf)
        mw_rf_pi34 = Sin(mw_pi_rf_rft, freq=f1_rf, amp=self.amp1_rf, phase=phase_rf)
        pulse_sequence_rf_tomo_pi34 = [carry] + [mw_rf_tomo_pi34] +[carry_pi13] + [mw_rf_pi34] + [zero_rf]
        ...
        # for N4
        pulse_sequence_rf_N4 = [zero_rf] + [mw_rf] + [zero_rf]
        mw_rf.duration = mw_pi_rf_rft
        name_rf = 'NRABI_pi_rf_N4'
        wave_rf = Waveform610(name_rf, pulse_sequence_rf_N4, offset=0, file_type=0, sampling=self.sampling_rf)# 0 for wfm file and 1 for pat file
        self.waves_rf.append(wave_rf)
        seq_rf.append(wave_rf, wait=True)
        ...
        for i,t in enumerate(self.tau):
            ...
            wave_rf = Waveform610(name_rf, pulse_sequence_rf, offset=0, file_type=0, sampling=self.sampling_rf)# 0 for wfm file and 1 for pat file
            self.waves_rf.append(wave_rf)
            seq_rf.append(wave_rf, wait=True) #for tomo 0
            seq_rf.append(wave_rf, wait=True) #for tomo pi13

            mw_rf_tomo_pi34.duration = t
            name_rf_tomo_pi34 = 'NRABI_%03i_pi34' % i
            #waves = IQWaveform(name, pulse_sequence, file_type=0) # 0 for wfm file and 1 for pat file
            wave_rf_tomo_pi34 = Waveform610(name_rf_tomo_pi34, pulse_sequence_rf_tomo_pi34, offset=0, file_type=0, sampling=self.sampling_rf)# 0 for wfm file and 1 for pat file
            self.waves_rf.append(wave_rf_tomo_pi34)
            seq_rf.append(wave_rf_tomo_pi34, wait=True) #for tomo pi34

            
            seq_rf.append(wave_rf, wait=True) #for tomo Pi
    ...
    traits_view = View(
        ...
        title='NuclearRabi 2 smiq and awg610',
        buttons=[],
        resizable=True,
        width =1400,
        height=800,
