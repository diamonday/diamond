...
class NODMR_awg(Pulsed):
    ...
    """awg520"""
    main_dir = String(
        "\nODMR", desc=".WFM/.PAT folder in AWG 610", label="Waveform Dir."
    )
    freq_awg = Range(
        low=1,
        high=1.0e9,
    ...
    """awg610"""
    main_dir_rf = String(
        "\nODMR", desc=".WFM/.PAT folder in AWG 610", label="Waveform_rf Dir."
    )
    phase_sin_rf = Range(
        low=-100.0,
        high=100.0,
    ...
    def generate_sequence(self):
        ...
        for f in self.freqs:
            ...
            )  # for trigger dead time of awg610
        ...
        if self.y1 is None:
            ...
            try:
                ...
                p = fitting.fit_multiple_lorentzians(
                    x, y, N, threshold=self.threshold * 0.01
