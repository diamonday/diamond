...
class NODMR_awg( Pulsed ):

    """NMR_e0 measurement, but change the mw freq."""
    """awg520"""
    main_dir = String('\NODMR', desc='.WFM/.PAT folder in AWG 610', label='Waveform Dir.')
    ...
    """awg610"""
    main_dir_rf = String('\NODMR', desc='.WFM/.PAT folder in AWG 610', label='Waveform_rf Dir.')
    ...
    start_freq = Range(low=1, high=2.6e9, value=45e6, desc='AWG frequency', label='start freq [Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    end_freq   = Range(low=1, high=2.6e9, value=55e6, desc='AWG frequency', label='end freq [Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    freq_step  = Range(low=1, high=2.6e9, value=100e3, desc='AWG frequency', label='freq step [Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    
    freqs      = Array()

    #fit parameters
    show_fit = Bool(False, label='show fit')
    N_peak = Trait('auto', String('auto', auto_set=False, enter_set=True), Int(10000., desc='Number of Lorentzians used in fit', label='N', auto_set=False, enter_set=True))
    ...
    get_set_items = Pulsed.get_set_items + [ 'freq_awg','freq_awg_rf', 'amp1', 'mw_pi', 'mw_pi_rf',  'start_freq',  'end_freq',  'freq_step', 
    'show_fit', 'fit_parameters', 'threshold', 'freq_fit', 'line_width_fit','N_peak'
    ]
    
    def __init__(self, pulse_generator, time_tagger, microwave, awg, awg_rf, **kwargs):
        self.dual = False
        super(NODMR_awg, self).__init__(pulse_generator, time_tagger, microwave, awg, awg_rf, **kwargs)
        self.show_fit = True
    ...
    def compile_waveforms(self):
        ...
        self.freqs = np.arange(self.start_freq, self.end_freq, self.freq_step)
        freqs = self.freqs/self.sampling
        ...
        mw_rf = Sin(mw_pi_rf, freq=f1_rf, amp=self.amp1_rf, phase=phase_rf)
        name_rf = 'NODMR_rf'
        ...
        for i,f in enumerate(freqs):
            ...
            mw2 = Sin(mw_pi, freq=f, amp=self.amp1, phase=phase)
            name = 'NODMR_%02i' % i
            #waves = IQWaveform(name, pulse_sequence, file_type=0) # 0 for wfm file and 1 for pat file
            pulse_sequence = [zero, mw2, zero]
            waves = IQWaveform(name, pulse_sequence, offset=0, file_type=0, sampling=self.sampling)# 0 for wfm file and 1 for pat file
            self.waves.append(waves[0])
            self.waves.append(waves[1])
    ...
    @on_trait_change('y1, show_fit', 'threshold' , 'N_peak')
    def _update_fit_parameters(self):
        if self.y1 is None:
            ...
            try:
                ...
                p = fitting.fit_multiple_lorentzians(x, y, N, threshold=self.threshold * 0.01) # number of resonance is set as 1
                #print('fit success')
                #print(p)
            ...
            Tabbed(
                VGroup(
                    ...
                    HGroup(
                        ...
                        Item('N_peak'),
                        Item('threshold'),
                        Item('freq_fit', style='readonly'),
                        Item('line_width_fit', style='readonly'),
                        Item('contrast_fit', style='readonly'),
                        label='Fit',
                        show_border=True,
                    ),
                    VGroup(
