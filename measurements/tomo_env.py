...
class Tomo(Pulsed):
    ...
    n_envelop = Range(
        ...
        high=100,
        value=2,
        desc="Dimension of the system",
        label="number of envelop",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    ...
    def compile_waveforms(self):
        ...
        for p in range(self.n_envelop):
            ...
            envelop = Envelop(
                duration=0, amp_li=self.amp_f[p], freq_li=ffx, phase_li=self.phase_f[p]
            )
            # mw = sine*envelop
            mw = envelop

            pulse_sequence = [zero, operatorx, operatory, mw, zero] + [
                idleforminpointrequirement
            ]
    ...
    def _create_line_plot(self):
        ...
        for i in range(self.n_envelop):
            plot.plot(
                ("index", "y%d" % i), color="auto", line_width=2, id="0", name="y%d" % i
            )
