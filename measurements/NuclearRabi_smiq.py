...
class NuclearRabi_smiq( Pulsed ):
    ...
    get_set_items = Pulsed.get_set_items + [ 'freq_awg_rf', 'amp1_rf', 'show_fit', 'fit_parameters', 'rabi_frequency', 'rabi_period',
        'rabi_offset', 'rabi_contrast', 'pi2', 'pi', 'pi32', 'mw_pi'
    ]
    
    def __init__(self, pulse_generator, time_tagger, microwave, awg_rf, **kwargs):
        self.dual = False
        super(NuclearRabi_smiq, self).__init__(pulse_generator, time_tagger, microwave, awg_rf, **kwargs)
        self.show_fit = True
    ...
    def generate_sequence(self):
        ...
        max_time = tau[-1] + trig_interval_rf + trig_delay_rf + mw_pi + mw_pi
