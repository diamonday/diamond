...
class DEER3pi2(Pulsed):
    ...
    cap_runs = Bool(True)
    cap_runs_margin = Range(
        low=1,
        high=99,
        value=5,
        label="Cap Runs Margin [%]",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    ...
    def apply_parameters(self):
        ...
        if not (
            ...
            self.count_data = np.zeros(
                (2 * len(frequencies) + 4 * self.references, n_bins)
            )
            self.run_time = 0.0
                ...
                np.max(
                    (
                        ...
                        int(
                            ...
                            * 1e9
                            / (
                                2 * (self.laser + self.wait + _max_flip_t)
                                + 4 * (self.t_mw1_3pi2 + self.tau)
                                + 100
                            )
                        ),
                    )
    ...
    def _run(self):
        ...
        try:
            ...
            _max_runs = int(
                self.sweeps_per_point * (1.0 - self.cap_runs_margin / 100.0)
            )
            ...
            while self.run_time < self.stop_time:
                ...
                for j in range(1 + self.randomize):
                    if self.randomize:
                        ...
                        _rand_mapping = list(range(0, _len))
                    ...
                    for i in range(_len):
                        ...
                        while _runs < _max_runs:
                            ...
                            if _iter >= 20:
                                logging.getLogger().warn(
                                    "Timed out. Insufficient measurement runs collected, RF Frequency: "
                                    + str(_freq)
                                )
                                break
                        ...
                        if self.randomize:
                            ...
                            logging.getLogger().debug(
                                "Data Insertion Position: " + str(_pos)
                            )
                ...
                if self.references:
                    ...
                    tagger = TimeTagger.Pulsed(
                        self.n_bins, int(np.round(self.bin_width * 1000)), 4, 0, 2, 3
                    )
                    ...
                    while _runs < _max_runs:
                        ...
                        if _iter >= 20:
                            logging.getLogger().warn(
                                "Timed out. Insufficient measurement runs collected for references"
                            )
                            break
            ...
            Tabbed(
                ...
                VGroup(
                    ...
                    HGroup(
                        ...
                        Item(
                            "cap_runs_margin", width=-40, enabled_when='state != "run"'
                        ),
                    ),
                    label="Others",
                ),
...
class DEERRabi3pi2(Pulsed):
    ...
    int_tau = Range(
        ...
        high=5000000.0,
        value=300.0,
        desc="Interaction Tau [ns]",
        label="Interaction Tau [ns]",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    ...
    mw2_frequency = Range(
        ...
        high=1000e6,
        value=200e6,
        desc="MW2 Frequency [Hz]",
        label="MW2 frequency [Hz]",
        mode="text",
        auto_set=False,
        enter_set=True,
    )
    ...
    def apply_parameters(self):
        ...
        self.tau = np.arange(
            self.tau_begin, self.tau_end + self.tau_delta, self.tau_delta
        )
        n_bins = int(self.record_length / self.bin_width)
        time_bins = self.bin_width * np.arange(n_bins)
    ...
    def _run(self):
        ...
        try:  # try to run the acquisition from start_up to shut_down
            ...
            tagger = TimeTagger.Pulsed(
                self.n_bins, int(np.round(self.bin_width * 1000)), _len * 2, 0, 2, 3
            )
            ...
            while self.run_time < self.stop_time:
                ...
                if self.randomize:
                    _rand_mapping = list(zip([0] * _len, list(range(0, _len)))) + list(
                        zip([1] * _len, list(range(0, _len)))
                    )
                    random.shuffle(_rand_mapping)
            ...
            Tabbed(
                VGroup(
                    ...
                    HGroup(
                        Item("mw2_power", width=-40, enabled_when='state != "run"'),
                        Item(
                            "mw2_frequency", width=-120, enabled_when='state != "run"'
                        ),
                        Item(
                            "tau_begin",
                            width=-80,
