...
class VQE(Pulsed):
    ...
    freq_rf_34 = Range(low=1, high=2.6e9, value=2.94025e+6, desc='radio frequency of energy gap between 3 and 4', label='frequency rf_34 [Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    ...
    def __init__(self, pulse_generator, time_tagger, microwave, awg, awg_rf, **kwargs):
        super(VQE, self).__init__(pulse_generator, time_tagger, microwave, awg, awg_rf, **kwargs)
    ...
    def VQE_wfm(self,i,j):
        ...
        if j == 0:
            ...
            #mw
            if int(t_2pi_rot_e/2 + rot_rf_mwt + t_2pi_rot_e/2 + t_2pi_rf_34_mwt/4) < 256:
                carry1 = Idle(0)
                carry1.duration = int(rot_rf_mwt) 
                carry2 = Idle(0)
                carry2.duration = int(t_2pi_rf_34_mwt/4) 

                pulse_sequence = [zero] + Pi_all_x + [carry1] + Pi_all_x + [carry2] + Pi_24 + [zero]
                name = 'VQE_%02i_%02i' % (i, j)
                sub = IQWaveform(name, pulse_sequence, offset=0, file_type=0, sampling=self.sampling)
                sub_waves.append(sub['I'])
                sub_waves.append(sub['Q'])

            elif int(t_2pi_rot_e/2 + rot_rf_mwt) < 256 < int(t_2pi_rot_e/2 + rot_rf_mwt + t_2pi_rot_e/2 + t_2pi_rf_34_mwt/4):
                sub = Sequence('VQE_%02i_%02i' % (i, j) )
                ...
                carry2 = Idle(0)
                n2, carry2.duration = int(t_2pi_rf_34_mwt/4 - stub) / 256, int(t_2pi_rf_34_mwt/4 - stub) % 256
                t_offset = t_b + n2 * 256
                ...
                carry1 = Idle(0)
                n1, carry1.duration = int(rot_rf_mwt - stub1) / 256, int(rot_rf_mwt - stub1) % 256
                ...
                name2 = 'VQE_%02i_%02i_2' % (i, j)
                wfm2 = IQWaveform(name2, pulse_sequence2, offset=t_offset1, file_type=0, sampling=self.sampling)
                sub.append(wfm2)
                sub_waves.append(wfm2['I'])
                sub_waves.append(wfm2['Q'])
                ...
                carry2 = Idle(0)
                n2, carry2.duration = int(t_2pi_rf_34_mwt/4 - stub2) / 256, int(t_2pi_rf_34_mwt/4 - stub2) % 256
                ...
                wfm3 = IQWaveform(name3, pulse_sequence3, offset=t_offset2, file_type=0, sampling=self.sampling)
            ...
            pulse_sequence_rf = [carry1_rf, rot_ny, rot_nx, carry2_rf] + pi2y_rf_p + [zero]
            name_rf = 'VQE_%02i_%02i_rf' % (i, j)
            sub_rf = Waveform610(name_rf, pulse_sequence_rf, offset=0, file_type=0, sampling=self.sampling_rf)
            sub_waves_rf.append(sub_rf)
            ...
            carry2_rf = Idle(0)
            carry2_rf.duration = int(t_2pi_rot_e_rft/2) 
            pulse_sequence_rf = [carry1_rf, rot_ny, rot_nx, carry2_rf] + pi2y_rf_m + [zero]
            name_rf = 'VQE_%02i_%02i_rf' % (i, j)
            sub_rf = Waveform610(name_rf, pulse_sequence_rf, offset=0, file_type=0, sampling=self.sampling_rf)
            sub_waves_rf.append(sub_rf)
            ...
            if int(t_2pi_rot_e/2 + rot_rf_mwt + t_2pi_rf_34_mwt/4) < 256:
                ...
                carry = Idle(0)
                n, carry.duration = int(rot_rf_mwt + t_2pi_rf_34_mwt/4 - stub) / 256, int(rot_rf_mwt + t_2pi_rf_34_mwt/4 - stub) % 256
                t_offset = t_b + n * 256
    ...
    get_set_items = Pulsed.get_set_items + ['check_state', 'spin_state', 'rot_n_y', 'rot_n_x', 'main_dir', 'main_dir_rf', 'amp1', 'amp1_rf', 'freq_rot_e', 'amp_rot_e', 't_2pi_rot_e', 'freq_rf_34', 'amp_rf_34', 't_2pi_rf_34', 'freq_mw_13', 'amp_mw_13', 't_2pi_mw_13', 'freq_mw_24', 'amp_mw_24', 't_2pi_mw_24']
