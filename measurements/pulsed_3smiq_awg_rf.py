...
class Pulsed( ManagedJob, GetSetItemsMixin ):
    ...
    # Directory storing .WFM/.PAT files in AWG
    # Default directory is \waves
    main_dir_rf = String('\waves', desc='.WFM/.PAT folder in AWG', label='Waveform Dir.:', mode='text')
    wfm_button_rf = Button(label='Load rf', desc='Compile waveforms and upload them to the AWG rf.')

    # Traits ########################
    reload_awg_rf = Bool(True, label='reload rf', desc='Compile waveforms upon start up.')
    upload_progress_rf = Float(label='Upload progress rf', desc='Progress uploading waveforms', mode='text')
    
    vpp_rf       = Range(low=0.0, high=2.0, value=0.2, label='vpp CH1 rf [V]', desc='Set output voltage peak to peak [V] of CH1.', mode='text', auto_set=False, enter_set=True)
    trig_interval_rf = Range(low=11.0, high=10000.0, value=40, label='Trig Interval rf', desc='Set the trigger duration for the AWG.', mode='text', auto_set=False, enter_set=True)
    trig_delay_rf = Range(low=11.0, high=10000.0, value=390, label='Trig Delay rf', desc='Trigger delay of the AWG.', mode='text', auto_set=False, enter_set=True)
    sampling_rf   = Range(low=50.0e3, high=2.6e9, value=50e6, label='sampling rf [Hz]', desc='Set the sampling rate of the AWG.', mode='text', auto_set=False, enter_set=True)
    waves_rf      = []
    ...
    # Sequence
    laser = Range(low=1., high=5e6, value=4000., desc='laser [ns]', label='laser [ns]', mode='text', auto_set=False, enter_set=True)
    ...
    power = Range(low=-100.,  high=25.,   value=0,      desc='microwave power',     label='power [dBm]',    mode='text', auto_set=False, enter_set=True)
    
    frequency2  = Range(low=1, high=20e9,  value=2.70e9, desc='SMIQ frequency 2', label='SMIQ frequency 2 [Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    power2 = Range(low=-100.,  high=25.,   value=0,      desc='microwave power 2',     label='power 2 [dBm]',    mode='text', auto_set=False, enter_set=True)
    
    frequency3  = Range(low=1, high=20e9,  value=2.70e9, desc='SMIQ frequency 3', label='SMIQ frequency 3 [Hz]', mode='text', auto_set=False, enter_set=True, editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    power3 = Range(low=-100.,  high=25.,   value=0,      desc='microwave power 3',     label='power 3 [dBm]',    mode='text', auto_set=False, enter_set=True)
    ...
    record_length                = Range(low=0,  high=10000, value=4000, desc='time window for pulse analysis [ns]',          label='profile length [ns]', mode='text', auto_set=False, enter_set=True)
    time_window_width            = Range(low=10, high=1000,  value=400,  desc='time window for pulse analysis [ns]',          label='signal window [ns]',  mode='text', auto_set=False, enter_set=True)
    ...
    time_window_offset_normalize = Range(low=0,  high=10000, value=2900, desc='offset of time window for normalization [ns]', label='normalize at [ns]',   mode='text', auto_set=False, enter_set=True)
    ...
    # get_set_items
    get_set_items = [
        'laser', 'wait', 'frequency', 'power','frequency2', 'power2','frequency3', 'power3', 'run_time', # later also sweeps
        'time_bins', 'count_data', 'bin_width', 'record_length',
        'pulse_profile','flank','time_window_width','time_window_offset_signal','time_window_offset_normalize',
        'tau', 'y1', 'y2',
        'nu', 'yf1', 'yf2',
        'vpp_rf', 'sampling_rf','trig_interval_rf', 'trig_delay_rf',
    ...
    def start_up(self):
        ...
        self.microwave2.setOutput(self.power2, self.frequency2)
        self.microwave3.setOutput(self.power3, self.frequency3)
    ...
    def shut_down(self):
        ...
        self.microwave2.setOutput(None, self.frequency2)
        self.microwave3.setOutput(None, self.frequency3)
        #self.awg.stop()
