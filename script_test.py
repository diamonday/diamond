...
def convertSequenceToBinary(sequence,loop=True):
        ...
        for channels, time in sequence:
            ...
            if repetitions > 0:
                ...
                buf += pg_test.pack(repetitions-1,ONES*bits) # rep=0 means the block is executed once
                print('rep:',[ord(s) for s in pg_test.pack(repetitions-1,ONES*bits)])
                print(bits)
            if index > 0:
                pattern = blank.copy()
        ...
        if loop: # repeat the hole sequence
            if index > 0: # fill up incomplete block with zeros and write it
                pg_test.setBits(pattern,index,CHANNEL_WIDTH-index,np.zeros(N_CHANNELS,dtype=bool))
                buf += pg_test.pack(0,pattern)
                print('bin:',[ord(s) for s in pg_test.pack(0,pattern)])
